...
const POSTS = [
...

  {
    ...
    body: [
      ...
      '這是最重要的一問。皮膚狀態決定了你需要的洗感方向 ── 不是「需要多保濕」這種抽象的方向，而是配方裡油脂組合應該偏哪一邊。',
      '敏感肌、痘痘困擾、瑕疵受損 → 看「花神守護」系列：海棠（修復）、金盞（舒緩）、綠豆（清芳）、蝶豆（清瑩）。這四款的設計取捨偏「溫和、低刺激」── 椰子油壓得低、橄欖跟乳油木果脂多一點，洗完不緊。',
      '極乾燥、熟齡、走過幾道刺激 → 霧蜜（蜜潤）、桂花（潤膚）。這兩款是「水分鎖得住」的方向 ── 蜂蜜跟桂花浸泡油把保濕拉到最高，配方偏厚實。',
      '油痘、混合肌、容易出油 → 綠豆、蝶豆。研磨綠豆粉跟蝶豆花的設計是「清而不剝」── 角質代謝有了，但底層保濕還在。',
      '一般肌、想日常用 → 大米（暖心）、酒粕（醉月）、桂花。這三款不偏不倚，洗感平衡，每天用都適合。',
      { type: 'h2', text: '第二問：你在哪個季節用？' },
      '季節影響皮膚需要什麼。冬天皮脂分泌少、空氣乾，保濕跟修復是優先；夏天油脂多、流汗多，清爽跟代謝是優先。',
      '冬天 / 換季 → 修復力強的：海棠、霧蜜、金盞、桂花。這四款放冬天的浴室裡，洗完皮膚不會像被抽水。',
      '夏天 / 流汗多 → 一皂到底·清爽款。荷荷芭油的脂肪酸跟皮脂同源，運動完油脂分泌變多時反而能起平衡 ── 不是用清潔力把油刮掉，是讓皮脂自己回到該有的位置。',
      '春秋 / 換季中間 → 桂花、大米、酒粕。這三款的洗感比較中性，季節不極端時用最舒服。',
      { type: 'h2', text: '第三問：你要洗哪個部位？' },
      '人不是一塊皮膚 ── 臉、身體、頭髮對 pH 的耐受度差很多。冷製手工皂的本質是鹼性（pH 8–10）；弱酸皂餅是 pH 5–6，跟肌膚原生酸鹼吻合。一塊皂走天下是浪漫，但對皮膚不是最好的。',
      '全身（軀幹、手腳） → 任何一款冷製皂都行。挑你看順眼、聞順鼻的那一塊。',
      '臉 → 看皮膚狀況：年輕健康肌冷製皂沒問題；中性偏乾或敏感則建議弱酸皂餅（茉莉沐膚），或冷製皂選最溫和的（海棠、金盞），洗完立刻保濕。',
      '頭髮 / 頭皮 → 山茶淨髮（弱酸髮餅）。鹼性會讓毛鱗片打開、頭髮澀，弱酸髮餅這條路比較友善。',
      '旅行 / 一塊到底 → 一皂到底（清爽款 / 保濕款）。設計就是為「一塊到底」── 全身都能洗，不需帶兩塊。',
      { type: 'h2', text: '一張對照表' },
      '把上面三問疊起來，最快的找法：',
      '冬天敏感肌全身用 → 海棠 / 金盞 / 霧蜜',
      '夏天油性肌全身用 → 蝶豆 / 綠豆 / 一皂到底·清爽',
      '日常洗臉 → 茉莉沐膚（皂餅，溫和）／海棠（冷製，敏感肌）',
      '日常洗頭 → 山茶淨髮（弱酸髮餅）',
      '一塊到底 → 一皂到底·清爽（夏）／保濕（冬）',
      '送禮 → 桂月流金 · 桂花（香氣最廣受喜歡）',
