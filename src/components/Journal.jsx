...
const POSTS = [
...

  {
    ...
    body: [
      '常常有人問：你們十二款皂、兩款餅，第一塊我該選哪個？',
      '老闆娘鍋邊的回答總是同一句：先別看花、先別看香 ── 看你自己。一塊對的皂，是肌膚、季節、跟用途三件事一起對上的結果。我們把這個方式整理成「選皂三問」，三個問題問完，你大概就知道該從哪一塊開始。',
      { type: 'h2', text: '第一問：你是什麼皮膚？' },
      '先別急著挑皂 ── 大家常說「我是混合肌」、「我是敏感肌」，但你真的知道自己屬於哪一類嗎？這一問問清楚了，後面才挑得對。',
      '醫美與皮膚科把肌膚大致分成五型：敏感、乾燥、油性、混合、一般。它們不是「好或壞」的等級，而是不同的特徵組合 ── 同一個人也會隨年齡、季節、壓力滑動。先看下面五段定義，再對照看自己最像哪一個。',
      '敏感肌 ── 屏障比較薄，容易泛紅、刺癢、對洗劑或保養品反應強。重點不是「皮膚不好」，是「容易被外界刺激」。換季、空調、高敏成分都可能觸發。台灣濕熱 + 空污環境下，敏感族群比想像中多。',
      '乾燥肌 ── 皮脂分泌不足、洗完緊繃、冬天脫屑、細紋早出現。一年四季都偏渴，特別怕熱水洗澡跟冷氣房整天。',
      '油性肌 ── 皮脂旺、T 字易亮、毛孔大、夏天容易長痘。年輕時最明顯；35 歲後皮脂分泌會慢慢下降，多數人會轉成混合。',
      '混合肌 ── T 字（額頭、鼻翼）油亮、雙頰偏乾。這是亞洲人最常見的型態。難在「同一張臉要分區處理」── 太滋潤悶 T 字、太清爽乾雙頰。',
      '一般肌 ── 油水平衡、不過油不過乾、屏障穩定。少數幸運兒，常見於孩童時期；成年後維持一般肌的人不多。',
      { type: 'illustration', kind: 'skin-types' },
      '不確定？做個小測試 ── 早上溫水洗完臉，什麼都不擦，等 30 分鐘看臉的狀態：兩頰緊繃就是偏乾、T 字發亮就是偏油、兩種同時出現就是混合、發紅或刺癢就是敏感、什麼都沒有就是一般。',
      '對應到金花樓十四款的選法：',
      '敏感肌、痘痘困擾、瑕疵受損 → 看「花神守護」系列：海棠（修復）、金盞（舒緩）、綠豆（清芳）、蝶豆（清瑩）。設計取捨偏「溫和、低刺激」── 椰子油壓得低、橄欖跟乳油木果脂多一點，洗完不緊。',
      '乾燥肌、熟齡、走過幾道刺激 → 霧蜜（蜜潤）、桂花（潤膚）。這兩款是「水分鎖得住」的方向 ── 蜂蜜跟桂花浸泡油把保濕拉到最高，配方偏厚實。',
      '油性肌、混合肌、容易出油 → 綠豆、蝶豆。研磨綠豆粉跟蝶豆花的設計是「清而不剝」── 角質代謝有了，但底層保濕還在。',
      '一般肌、想日常用 → 大米（暖心）、酒粕（醉月）、桂花。這三款不偏不倚，洗感平衡，每天用都適合。',
      { type: 'h2', text: '第二問：你在哪個季節用？' },
      '七月中午，慢跑一小時回家。十二月寒流，洗完手連擦護手霜都來不及。同一個人，這兩個畫面需要的皂，配方差很多。',
      '我們不是先想季節再想皂，而是反過來 ── 先想原料能做什麼，再對應到什麼天氣最舒服。',
      '適合夏天的原料：荷荷芭油、薰衣草粉、綠豆粉、蝶豆花。為什麼？荷荷芭油的脂肪酸結構幾乎跟人體皮脂同源 ── 流汗油多時不是把油刮乾，而是讓皮脂自己回到平衡。薰衣草粉帶清涼草本氣，洗完皮膚有微涼感。綠豆粉幫角質代謝，毛孔不容易堵。蝶豆花泡沫輕盈，洗起來不悶。',
      '適合冬天的原料：蜂蜜、桂花浸泡油、乳油木果脂、瓊崖海棠油。為什麼？蜂蜜是天然保濕因子，能直接抓住空氣裡的水分。乳油木果脂洗完會留一層薄薄的脂膜鎖住水。瓊崖海棠油修復受損屏障 ── 冬天皮膚最常出現的「乾癢」其實是屏障破洞。桂花浸泡油讓配方厚實，撐起一塊耐洗、保濕的皂體。',
      '春秋換季：金盞花浸泡油、米糠油、酒粕。為什麼？這幾支都偏溫和、不刺激。換季是皮膚最敏感的時候，氣溫起伏大、皮脂分泌節律亂，這時候用厚重的修復皂太悶、用清爽款又不夠 ── 中性溫和的這幾款剛好過渡。',
      '把原料放回生活場景，立刻就有畫面：',
      '「七月慢跑回家想沖個澡」→ 一皂到底·清爽款（荷荷芭 + 薰衣草粉）。汗水、油、夏日體味一塊洗掉，沖完還有一點涼意。',
      '「十二月寒流，洗完手不擦油就乾裂」→ 桂花潤膚或霧蜜（蜂蜜 + 桂花浸泡油）。洗完雙手不需要急著擦護手霜，皂體自己留下一層滋潤。',
      '「過年回中部換水換氣候，臉開始發癢」→ 金盞舒緩（金盞花浸泡油）。換水換空氣換床單，皮膚最不耐煩的那幾天，金盞花溫和過渡。',
      { type: 'h2', text: '第三問：你要洗哪個部位？' },
      '人體不是只有一塊肌膚 ── 它是好幾種肌膚的綜合體。臉的角質薄、頭皮的毛囊密、軀幹的皮脂厚、手腳的角質又比軀幹厚 ── 同一塊皂洗到不同地方，效果完全不一樣。',
      '我們從三個常被問的生活問題切入 ── 答案會把你帶到該選哪一塊。',
      'Q：為什麼用開架洗臉乳洗完臉超乾，洗手卻剛剛好？',
      'A：因為臉部皮脂膜薄、皮脂分泌的節律也跟手不同。同一支洗劑帶走的水分，手部扛得住、臉部就是過頭。這也是為什麼很多人會說「我的洗面乳跟洗手乳不一樣」── 不是行銷話術，是真有道理。',
      'Q：為什麼有人說「洗冷製皂頭髮會澀」？',
      'A：頭髮的毛鱗片對 pH 特別敏感。冷製皂是鹼性（pH 8–10），鹼會把鱗片打開、髮絲糾結粗糙。除非洗完再用酸性收尾（檸檬水、米醋茶這類「酸洗」），不然直接拿冷製皂洗頭就是會澀 ── 這不是皂不好，是它本來就不為頭髮設計。',
      'Q：為什麼有人洗冷製皂全身沒事，洗到臉就緊？',
      'A：臉跟身體對 pH 的耐受度差距比想像大。臉皮脂膜薄、恢復力慢；身體厚、恢復快。同一塊皂從上洗到下，臉會先抗議。',
      '從這三個問題看，皂不能一塊走全身。我們把皂分成三條路：',
      '全身（軀幹、手腳） → 任何一款冷製皂都行。挑你看順眼、聞順鼻的那一塊。',
      '臉 → 看皮膚狀況：年輕健康肌冷製皂沒問題；中性偏乾或敏感肌建議弱酸皂餅（茉莉沐膚），或冷製皂選最溫和的（海棠、金盞），洗完立刻保濕。',
      '頭髮 / 頭皮 → 山茶淨髮（弱酸髮餅）。pH 5–6 跟頭皮原生酸鹼吻合，洗完不需要再做酸洗收尾。',
      '旅行 / 一塊到底 → 一皂到底（清爽款 / 保濕款）。設計就是為「全身一塊洗」── 不需要帶兩塊。',
      '一張完整的洗澡日常，可能會用到兩種：身體用冷製、頭髮用弱酸髮餅、臉視膚況選擇。皂餅不是取代手工皂，是各自負責不同部位。',
      { type: 'h2', text: '一張對照表' },
      '把上面三問疊起來，最快的找法（每行後面附「為什麼」，懶得讀全文也能一眼看懂）：',
      '冬天敏感肌全身用 → 海棠 / 金盞 / 霧蜜｜瓊崖海棠油修復屏障、長濱金盞花溫和舒緩、霧峰蜂蜜深度鎖水',
      '夏天油性肌全身用 → 蝶豆 / 綠豆 / 一皂到底·清爽｜蝶豆花抗氧化提亮、綠豆粉幫角質代謝、荷荷芭油平衡皮脂',
      '日常洗臉 → 茉莉沐膚（皂餅，溫和）／ 海棠（冷製，敏感肌）｜茉莉沐膚 pH 5–6 接近肌膚原生酸鹼；海棠的瓊崖海棠油最修復',
      '日常洗頭 → 山茶淨髮（弱酸髮餅）｜pH 5–6 不打開毛鱗片、山茶花油給髮絲輕脂、蠶絲蛋白順髮',
      '一塊到底（旅行 / 運動）→ 一皂到底·清爽（夏）／ 保濕（冬）｜荷荷芭油同源皮脂全身都能洗，固體不漏、機場安檢沒問題',
      '送禮 → 桂月流金·桂花｜桂花浸泡油的甜香不挑人、配方溫和、視覺米黃帶桂花橙點，外觀好看',
      { type: 'h2', text: '選定一塊，用四週看看' },
      '冷製皂的有趣之處在於它不是工業洗劑那種「一試立刻知道好不好」的東西。它跟你的皮膚有相性 ── 同一塊皂，不同人用感受可以差很多。',
      '建議的方式：選定一塊，每天用，連用四週。皮膚自己會告訴你答案。如果四週後皮膚狀態變平穩、洗完不緊、香氣讓你願意一直拿起來 ── 這就是你的那一塊。如果四週後感覺微微緊或乾，換鄰近一級（例如海棠 → 霧蜜，再厚實一檔）。',
      '一塊皂用完大約兩個多月。慢慢洗、慢慢挑。',
      {
        type: 'faq',
        items: [
          {
            q: '我同時是敏感肌又油性，該選哪一邊？',
            a: '優先選「溫和」這邊 ── 蝶豆或綠豆。這兩款的設計兼顧角質代謝跟低刺激，比較不會在洗完之後反過來刺激出更多油。',
          },
          {
            q: '我要怎麼知道一塊皂適不適合我？',
            a: '看洗完當下與半小時後的感受。「洗完緊繃、半小時恢復」是普通洗感；「洗完緊繃、需要立刻擦保養品才舒服」是這塊皂偏鹼太多 / 你皮膚需要更溫和的方向。觀察兩三次就有結論。',
          },
          {
            q: '一塊皂可以同時洗臉跟身體嗎？',
            a: '冷製皂技術上可以，但臉的皮膚比較薄，鹼性的衝擊會比身體明顯。如果你洗臉只有「一點點緊」可以接受，那就一塊到底；如果緊得不舒服，分開兩塊（身體用冷製、臉用弱酸皂餅）會比較好。',
          },
          {
            q: '我想送禮，該選哪一款？',
            a: '送禮優先選香氣討喜、不挑膚質的款式。桂花（潤膚）跟霧蜜（蜜潤）這兩款最常被回購當禮物 ── 香氣不挑人，配方溫和，外觀也好看。如果送的對象有頭皮敏感問題，可以加一塊山茶淨髮髮餅一起送。',
          },
          {
            q: '我看不懂 INS 值、五力這些詞，會影響我選嗎？',
            a: '不會。這篇就是不用懂那些數字也能挑的方式 ── 從你的皮膚、季節、用途三件事入手就夠了。如果想深入了解配方背後的數字怎麼算，可以連到〈INS 值 ── 老闆娘看配方第一個看的數字〉跟〈脂肪酸與五力〉兩篇。',
          },
          {
            q: '我想搭一組三塊送禮，怎麼挑？',
            a: '建議「日常香氣 + 冬天修復 + 夏天清爽」三層。例：桂花（香氣不挑人）+ 海棠（敏感肌都能用）+ 一皂到底·清爽（運動或夏日）。或加一塊山茶淨髮讓對方頭髮也用得到。送禮的核心是「他都用得到」── 三塊覆蓋不同情境最保險。',
          },
        ],
      },
    ],
