...
const POSTS = [
...

  {
    ...
    body: [
      ...
      '冬天敏感肌全身用 → 海棠 / 金盞 / 霧蜜｜瓊崖海棠油厚實潤澤、長濱金盞花溫和、霧峰蜂蜜的天然糖份鎖水',
      '夏天油性肌全身用 → 蝶豆 / 綠豆 / 一皂到底·清爽｜蝶豆花走清透洗感、綠豆粉幫角質代謝、荷荷芭油平衡皮脂',
      '日常洗臉 → 茉莉沐膚（弱酸皂餅）／海棠（冷製，敏感肌）｜茉莉沐膚 pH 5–6 接近肌膚原生酸鹼；海棠最厚潤',
      ...
      '四週後皮膚平穩、洗完不緊、每天願意拿起來，那就對了。如果還是微微緊或乾，換鄰近一級（例如海棠 → 霧蜜往厚實移一檔）。一塊大概用兩個多月，慢慢洗、慢慢挑。',
      {
        ...
        items: [
          ...
          {
            q: '送禮三塊組怎麼搭？',
            a: '「日常香氣 + 冬天滋養 + 夏天清爽」三層最保險。例：桂花（不挑人）+ 海棠（敏感肌都行）+ 一皂到底·清爽（運動 / 夏日）。對方頭皮敏感再加一塊山茶淨髮。送禮核心是「他都用得到」 ── 三塊覆蓋不同情境最不會 NG。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-11',
    cover: {
      src: '/images/journal/taiwan-water.png',
      alt: '工坊木桌上兩只手作陶碗並排 ── 一碗清水起著輕盈大泡、一碗水濁起著緊密小泡，中間放著皂與木匙',
      fallback: { tone: 'cool', flower: 'pine' },
    },
    description:
      '台灣北部偏軟水、南部偏硬水 ── 同一塊冷製皂在不同城市洗起來感受差很多。本篇講硬水軟水對皂泡、洗感、皂體消耗的影響，以及軟水族跟硬水族各自的對策。',
    keywords: ['軟水硬水', '台灣水質', '皂泡', '冷製皂', '皂感差異', '水質與洗澡'],
    related: ['why-handmade-soap', 'how-to-choose-soap', 'soap-storage'],
    body: [
      '老闆娘鍋邊有一個朋友從台北搬到高雄。一個月後傳訊來：「你們的皂是不是換配方了？泡沫變得很怪」。',
      '配方一樣。換的是水。',
      '台灣這座島從北到南、從西到東水質差異不小 ── 同一塊皂在台北洗起來輕盈、到高雄變得「硬一點」、到台東又是另一回事。這不是皂的問題，是水跟皂之間化學反應的差別。',
      { type: 'h2', text: '軟水跟硬水的差異' },
      '水的「硬度」指的是水裡面鈣、鎂等礦物質離子的濃度。礦物質少 = 軟水；礦物質多 = 硬水。',
      '硬水洗皂時會發生兩件事：',
      '一、礦物質跟皂裡的脂肪酸鈉鹽結合，生成「皂垢」（soap scum，俗稱浴室那層白霧或鈣化物）── 浪費掉一部份的皂、洗感變澀。',
      '二、泡沫變得緊密小但少 ── 不是皂打不出來，是被礦物質「綁架」一部份。同樣份量的皂，在硬水區能用的「有效泡沫」少。',
      '軟水洗皂相反 ── 泡沫蓬鬆、又輕又多、皂消耗得「快」（因為起泡更容易，皂表面溶得也快）。',
      {
        type: 'figure',
        src: '/images/journal/inline/taiwan-water-1.png',
        alt: '台灣島輪廓手繪示意 ── 北部標一個小水滴（軟）、南部標一個大水滴（硬）、中央山脈用波浪線示意',
        caption: '北軟南硬 ── 中央山脈是分水嶺，也是水質的分界',
      },
      { type: 'h2', text: '台灣南北的水質地圖' },
      '北部（基隆、台北、新北、桃園、新竹大部分）── 偏軟水。水源主要來自雪山山脈跟北部水庫，礦物質相對少。冷製皂在這裡洗起來最「順」，泡沫蓬、消耗也快（一塊用兩個月可能變一個半月）。',
      '中部（苗栗、台中、彰化、南投）── 中等水質。山泉水區（南投、苗栗山區）偏軟、平原都會區偏中性偏硬。',
      '南部（雲林、嘉義、台南、高雄、屏東）── 偏硬水。水源混合石灰岩地層流出的水、加上水庫蒸發濃縮，礦物質比例高。同一塊皂在這裡洗起來泡沫顯得「實在」但量少，皂垢比較容易留在浴室磁磚。',
      '東部（宜蘭、花蓮、台東）── 軟硬不一，多數地區水質好且偏軟（山泉水豐沛）。是台灣洗皂體驗最舒服的區域之一。',
      '離島（澎湖、金門、馬祖）── 多用海水淡化或地下水，硬度偏高。洗冷製皂會明顯感覺泡沫吃力。',
      { type: 'h2', text: '對皂的影響跟對策' },
      '軟水族（北部、東部、山區）── 皂消耗快，學著「省著用」。每次只搓出需要的份量、皂盤瀝水做好（不然容易整塊化）。也可以選配方稍硬的款 ── 例如本舍棕櫚比例正常的款（杜康醉月、稻花暖心），比起 70% 橄欖的純橄欖皂耐用。',
      '硬水族（南部、離島）── 皂泡沫吃力、皂垢明顯。對策：',
      '一、選清潔力較高的款。本舍的綠豆清芳、蝶語清瑩、一皂到底·清爽款月桂酸比例稍高，硬水區起泡比較不費力。',
      '二、洗澡前做一個小動作 ── 蓮蓬頭先沖一下身體，讓皮膚先濕透再搓皂。乾搓硬水裡的皂特別澀。',
      '三、浴室磁磚的皂垢用稀釋醋（白醋 1：水 5）擦掉，省得越積越厚。',
      '四、可以選擇裝個簡單的軟水器（淋浴頭那種小型款），有感降低硬度 ── 但這是另一筆預算的事，不是必要。',
      { type: 'h2', text: '頭髮對水質特別敏感' },
      '冷製洗髮皂在硬水區洗起來會明顯「澀」 ── 因為皂垢留在毛鱗片之間。這也是為什麼本舍選擇做弱酸性的山茶淨髮髮餅 ── 它不是皂、不會生皂垢，硬水軟水都洗得順。',
      '南部朋友如果只能選一個改變，是：頭髮從冷製洗髮皂換成弱酸髮餅。差別會非常明顯。',
      '一塊皂的洗感，不只是配方寫了什麼 ── 還有它跟你家水龍頭裡那條水的關係。',
      {
        type: 'faq',
        items: [
          {
            q: '我怎麼知道我家是軟水還硬水？',
            a: '簡單測法：白色磁磚或玻璃淋浴間水乾掉之後有沒有白色霧狀痕跡。明顯有 → 偏硬水。完全沒有 → 偏軟水。介於兩者 → 中等。',
          },
          {
            q: '硬水區的人應該避免冷製皂改用沐浴乳嗎？',
            a: '不需要。沐浴乳的界面活性劑也會跟硬水反應（只是反應產物不同）。重點是換洗法 ── 先濕身再搓皂、選擇清潔力稍強的款、勤擦皂垢。',
          },
          {
            q: '搬家前後皮膚變很乾，是水的關係嗎？',
            a: '可能是。水質變化（特別從硬水到軟水反過來不容易適應）會讓皮脂膜重整，2–4 週才會穩定。建議搬家後第一個月用最溫和的款（海棠或大米），讓皮膚適應新水。',
          },
          {
            q: '雨水或溪水洗皂特別舒服是真的嗎？',
            a: '是。雨水礦物質極低（接近蒸餾水），溪水如果是上游山泉也偏軟。古人在溪邊洗衣覺得衣服特別乾淨，是有化學原因的。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-08',
    cover: {
      src: '/images/journal/after-sweat.png',
      alt: '工坊木桌上一塊皂躺在折好的白色棉巾上，旁邊是一杯帶水珠的冷水、一只盛著薄荷葉的陶碗',
      fallback: { tone: 'cool', flower: 'mugwort' },
    },
    description:
      '為什麼流汗後 30 分鐘內洗澡最舒服？汗液在皮膚上停留越久，鹽分跟尿素結晶後越刺激，毛孔也跟著越堵。本篇講運動後的洗澡節奏、水溫選擇、跟適合的皂款。',
    keywords: ['運動後洗澡', '流汗肌膚', '30 分鐘黃金窗', '夏天清潔', '運動皂'],
    related: ['travel-soap', 'taiwan-water', 'humid-island-feet'],
    body: [
      '夏天晚上慢跑回家、騎完一段山路、戶外活動曬了一整天 ── 身體被汗濕透的時候，最想做的事就是沖一個澡。',
      '這個直覺是對的。流汗後身體有一個「黃金窗」── 大約 30 分鐘 ── 在這個時間內洗澡最舒服、對皮膚最好。',
      { type: 'h2', text: '為什麼是 30 分鐘' },
      '汗液剛從汗腺出來時是中性偏弱酸的水溶液 ── 99% 是水。但汗水接觸到空氣之後開始變化：水分蒸發、鹽分跟尿素濃縮、皮膚表面細菌開始把汗水代謝成有氣味的化合物。',
      '前 30 分鐘汗液還是相對溫和的、容易沖掉的。30 分鐘到 2 小時之間，鹽結晶開始刮皮膚（特別是鎖骨、脖子、後背的衣服摩擦處），尿素也開始分解產生氨味。超過 2 小時不洗，毛孔已經被反覆乾濕的皮屑跟汗鹽堵住一層 ── 隔天臉容易冒痘、背容易長疹。',
      {
        type: 'figure',
        src: '/images/journal/inline/after-sweat-1.png',
        alt: '一條時間軸手繪示意 ── 從跑步剪影出發、經過 30 分鐘的小時鐘標記、終於走到一塊皂',
        caption: '30 分鐘的窗口 ── 流汗到洗澡之間的最佳節奏',
      },
      { type: 'h2', text: '不同情境的洗澡節奏' },
      '輕度運動（30 分鐘走路、瑜伽、慢慢騎自行車）── 衣服微濕、頭皮微汗。回家可以先脫下濕衣換乾衣、喝水、休息 10 分鐘再洗。不急。',
      '中度運動（跑步、快騎、健身房）── 衣服明顯濕、需要大量補水。30 分鐘內洗澡最好；若辦公室或健身房有淋浴，當下沖會比帶汗回家舒服 5 倍。',
      '重度活動（戶外曬一整天、登山、海邊）── 不只汗，還有紫外線、海鹽、灰塵。回家立刻溫水沖一遍，先把表層髒沖掉再用皂洗第二輪。一次洗到底反而傷皮膚。',
      { type: 'h2', text: '水溫的選擇' },
      '直覺：流汗後想沖冷水。但生理建議是「溫水」──35–38°C，比體溫稍低一點。',
      '原因：剛運動完毛細血管擴張、心跳還快。突然冷水會讓血管急速收縮，對心血管不友善（特別是中老年）。溫水讓身體緩降，舒服又安全。冷感留給最後 30 秒：洗完用冷水沖一下手腳腳跟，毛細血管收縮一次，特別清醒。',
      { type: 'h2', text: '適合的皂款' },
      '一皂到底·清爽款 ── 為這個情境設計的款。荷荷芭油接近皮脂、薰衣草粉幫角質代謝、洗完帶一點涼。夏天慢跑回家用這塊，洗 5 分鐘汗、味、表層全部乾淨。',
      '綠豆清芳·澡豆 ── 油性肌或痘痘困擾的人。研磨綠豆粉幫毛孔代謝，運動後容易冒痘的人用這款連續一週會有感。',
      '蝶語清瑩·蝶豆 ── 重度紫外線曝曬後（戶外、海邊）。曬了一整天、流汗又黏，想要一塊洗起來乾淨清爽、不緊繃的皂。蝶豆花入皂走清透路線 ── 不是治療，是讓悶熱一天後的皮膚舒服收尾。',
      '不適合：純橄欖、霧蜜、桂花這些「保濕重型」的款。它們是好皂，只是不為「快速大量洗汗」設計，運動後用會覺得「沒洗乾淨」。',
      '流汗是身體在對你做事。洗澡是接著對身體說「謝謝、收工了」。',
      {
        type: 'faq',
        items: [
          {
            q: '我運動完很累，30 分鐘就累得不想動，怎麼辦？',
            a: '先脫濕衣、擦汗、補水，這個順序躺著也能做。然後 1 小時內洗也還在合理區間 ── 真正不可接受的是「衣服濕著睡著到隔天」。最低標：別讓汗在身上過夜。',
          },
          {
            q: '夏天每天都要洗兩次澡，會不會太傷皮膚？',
            a: '不會 ── 前提是用對皂。早上溫和款（大米、海棠）、晚上運動後清爽款（一皂到底·清爽、綠豆）。重點不是次數，是配方夠不夠溫和。',
          },
          {
            q: '健身房沒空洗澡，回家路上有 1 小時通勤怎麼辦？',
            a: '帶一條濕巾擦掉脖子、鎖骨、額頭三個重點區域。回家後立刻洗澡（不要做別的事）── 這是最常見的「黃金窗錯過」原因。',
          },
          {
            q: '冬天運動完還需要 30 分鐘黃金窗嗎？',
            a: '需要，但時間可以放寬到 60 分鐘 ── 冬天溫度低、汗液蒸發慢、刺激形成也慢。但「不要讓汗在身上過夜」這條規則一年四季都成立。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-09',
    cover: {
      src: '/images/journal/travel-soap.png',
      alt: '工坊木桌上一塊皂用蜜蠟布包好放在小木盒旁，旁邊配著手帕與一副圓框眼鏡 ── 一份輕便的旅行小裝備',
      fallback: { tone: 'cool', flower: 'mugwort' },
    },
    description:
      '為什麼旅行帶一塊皂比帶三瓶旅行裝沐浴乳更划算？機場安檢無痛、行李重量減半、塑膠瓶減量。本篇講帶皂出門的三個情境（機場、健身房、露營）跟三種包法（蜜蠟布、矽膠盒、舊紙包）。',
    keywords: ['旅行皂', '出差用皂', '健身房肥皂', '輕便旅行', '蜜蠟布', '塑膠減量'],
    related: ['soap-storage', 'after-sweat', 'why-handmade-soap'],
    body: [
      '出差包、健身包、旅行行李箱裡，本來都會塞兩三瓶塑膠瓶 ── 沐浴乳一瓶、洗髮精一瓶、洗面乳一瓶。每瓶 100 ml 旅行裝，加起來 300 g 重量、裝在透明塑膠袋裡過機場安檢、最後可能漏在行李箱底層。',
      '一塊冷製皂 105 g、固體、不漏液 ── 同樣是清潔但只佔三分之一的重量、零分的安檢摩擦、可以用兩三趟旅行還沒用完。',
      { type: 'h2', text: '為什麼旅行用皂特別划算' },
      '航空法規對固體沒有 100 ml 限制 ── 一塊皂直接放隨身包、託運包都行，不用裝透明袋、不用申報。國際線特別有感，過 TSA / 歐洲海關幾乎沒人會多看一眼。',
      '重量真的差很多。三瓶 100 ml 旅行裝平均 350 g（瓶子+液體），一塊皂 105 g。一個七天旅行的隨身行李，一塊皂幫你省下 245 g 給其他東西。',
      '塑膠減量是另一面。一個常旅行的人一年用掉的旅行裝塑膠瓶，少說 20–30 個 ── 換成一塊皂就完全不需要。',
      { type: 'h2', text: '三個情境' },
      '機場 / 海關 → 一皂到底·清爽款（夏）或保濕款（冬）。配方為「全身能洗、頭髮也行（短期）」設計，飛機上時差洗澡用一塊就夠。',
      '健身房 → 一皂到底·清爽款。流汗後快速沖澡，泡沫起得快、沖得乾淨、不會在置物櫃裡發霉（搭配蜜蠟布保管）。',
      '露營 / 戶外 → 一皂到底·清爽款 + 山茶淨髮髮餅兩件組。山裡的水通常偏軟、皂泡很豐 ── 是體驗冷製皂最好的場合。注意：洗澡水請遠離溪流、生物降解性好的冷製皂也不該直接污染水源。',
      '短住宿（民宿、Airbnb）── 帶一塊自己的皂，不用擔心房間擺的洗劑你不適應。',
      {
        type: 'figure',
        src: '/images/journal/inline/travel-soap-1.png',
        alt: '左邊三瓶塑膠瓶、右邊一塊皂用布包起來 ── 兩種旅行清潔組合的對比手繪示意',
        caption: '一塊頂三瓶 ── 重量、塑膠、安檢摩擦都減半',
      },
      { type: 'h2', text: '怎麼帶一塊皂出門' },
      '蜜蠟布（推薦）── 一塊正方蜜蠟布把皂包起來，邊角折一下就密合。不漏、不黏、可重複使用。本舍出貨會附一塊小尺寸蜜蠟布給常旅行的客人。',
      '矽膠盒 ── 防滑、好洗、有蓋子。但通氣不夠，回家要打開晾乾否則皂底會軟。適合短期 1–3 天的出差。',
      '舊紙包 ── 本舍的皂出貨用未漂紙包裹。直接拿這層紙當旅行包裝最有意境，但只能用一次（紙會吸水）。',
      '不要做的事：把濕的皂直接放回行李箱沒有任何包裝。會把衣服弄一塊。',
      '一塊皂可以走的地方比你想的多。',
      {
        type: 'faq',
        items: [
          {
            q: '皂在飛機行李箱裡會不會碎？',
            a: '冷製皂的硬度跟一塊優格差不多 ── 不易碎但會壓凹。蜜蠟布或矽膠盒包好就沒事。本舍的皂熟成 42 天以上、硬度足，曾經被客人寄到歐洲沒事。',
          },
          {
            q: '一塊皂能撐多久旅行？',
            a: '一個人天天洗一次澡用一塊 105 g 皂大約 60–80 天 ── 一趟旅行用 1/4 不到。帶一塊出門基本上夠用一整年的旅行（每次帶同一塊回家繼續用）。',
          },
          {
            q: '機場安檢真的不會被攔嗎？',
            a: '固體類在 TSA / 歐洲 / 亞洲幾乎不會被攔。只要不是「液體膏狀」（例如沒乾的香皂膏）就 OK。本舍的冷製皂全是固態，沒攔過。',
          },
          {
            q: '濕的皂帶回家如何處理？',
            a: '回家立刻打開包裝、放在浴室皂盤上自然乾燥 24 小時。蜜蠟布也攤開晾乾備用。這樣下一次出門又是新狀態。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-12',
    cover: {
      src: '/images/journal/soap-storage.png',
      alt: '工坊木桌上一只木條皂盤，皂塊靜靜放在上頭、水珠掛在木條間',
      fallback: { tone: 'warm', flower: 'pine' },
    },
    description:
      '一塊冷製手工皂能用多久，七成看你怎麼放它。浴室潮濕、皂盤選錯、隨手丟在水池邊 ── 三件事任一件做不對，原本可以洗兩個月的皂三週就化掉。本篇講皂盤、瀝水、儲存的物理原理跟實際做法。',
    keywords: ['手工皂保存', '皂盤推薦', '皂的壽命', '浴室收納', '皂的家', '冷製皂保存'],
    related: ['why-handmade-soap', 'travel-soap', 'last-sliver'],
    body: [
      '剛從本舍寄出去的皂，第一個任務不是洗澡 ── 是找到它的家。',
      '冷製手工皂跟工業洗劑不一樣。它沒有添加合成防腐劑、沒有抗濕氣處理 ── 純粹靠配方裡的脂肪酸跟甘油在做事。這也是它溫和的理由，但代價是「怕水」── 持續泡在水裡的皂會持續軟化、持續流失油脂。',
      '一塊冷製皂如果擺對地方，可以洗兩個月；擺錯地方，三週就化成一灘奶醬。差別只在「皂盤跟濕度」這一件小事。',
      { type: 'h2', text: '為什麼皂會「化掉」' },
      '化掉的本質是「持續接觸水」。水把脂肪酸鈉鹽（皂的本體）跟甘油（保濕分子）重新溶解 ── 等於每次都在把皂洗掉一點。',
      '解法只有一個：洗完之後讓它快速乾燥。讓皂底跟皂盤之間有空氣循環、讓殘水順利流走、讓濕度別一直泡著。做到這三件事，一塊皂的壽命就會是兩倍。',
      {
        type: 'figure',
        src: '/images/journal/inline/soap-storage-1.png',
        alt: '木條皂盤側面手繪示意 ── 皂塊靜置上頭、水珠透過縫隙落入下方淺盤、底下一道氣流箭頭',
        caption: '木條皂盤的工作原理 ── 水流走、空氣進來',
      },
      { type: 'h2', text: '皂盤的選擇' },
      '木條盤（推薦）── 條狀木塊間留縫隙，水順著縫隙落入下方淺盤，皂底接觸面積小、空氣對流好。檜木、柚木、竹編都可以。本舍工坊用的是台灣檜木，自己也做過送禮版本。',
      '陶瓷盤（中等）── 看起來最像「肥皂盤」，但底部如果是平面、又沒有瀝水孔，水會積在皂底，是化皂最快的設計。買的時候挑底部有凸起紋路或瀝水孔的款式。',
      '矽膠盤（中等）── 防滑、好洗，但軟材質容易積水。挑形狀有刻意做高點低點落差的款式。',
      '磁吸式（適合潔癖者）── 鋁製或不鏽鋼底貼磁鐵在牆上，皂塊嵌一顆小磁釘，懸空整塊。乾得最快但要習慣每次按嵌的動作。',
      '不要做的事：直接把皂丟在水池邊、丟在水龍頭附近、丟在洗手台陶瓷面凹處。這些位置永遠都有殘水。',
      { type: 'h2', text: '浴室潮濕的應對' },
      '台灣浴室一年四季都濕。即使皂盤對了，整個浴室在濕度 80% 以上時皂還是會慢慢吸水。對策有三：',
      '第一、每天洗完皂後甩兩下水。皂表面那層水珠是延壽最大的敵人，甩掉立刻乾。',
      '第二、長時間不用就請出浴室。出差超過三天、或週末回老家，把皂放回原本的紙盒、移到櫃子裡。冷製皂在乾燥的櫃子裡可以放半年沒事。',
      '第三、把多塊皂分開存放。一次買 6 塊不要一起拆封，用一塊拆一塊。其他 5 塊在原始紙包裹裡放抽屜，每塊都能再熟成一段時間 ── 越熟成的皂洗起來越溫和、越不易化（脂肪酸已經完整反應）。',
      { type: 'h2', text: '一塊皂的壽命' },
      '一塊金花樓的皂約 105 g，如果是日常洗澡頻率（一天一次、全身），加上皂盤對的話，能用 60–80 天。如果同時用兩塊（早洗臉用 A、晚洗澡用 B），各 100 天上下 ── 因為每塊都有充分乾燥的時間。',
      '用到剩 1/4 怎麼辦？兩條路：',
      '一、用完它。最後一段不好抓，可以裝進「皂袋」（mesh drawstring soap-saver bag，本舍有附），泡水搓出泡再洗。',
      '二、跟下一塊接力。新皂泡一下水，把舊皂的小角壓在新皂上，自然黏住。這個方式叫「皂接龍」，是傳統用皂法。',
      '另一個延伸閱讀：〈一塊皂的告別〉裡寫到怎麼處理用到剩一小角的皂塊，包括皂泥、皂袋、與惜物的儀式。',
      '一塊皂的壽命，從你怎麼放它的第一天就開始決定了。',
      {
        type: 'faq',
        items: [
          {
            q: '皂盤一定要選最貴的嗎？',
            a: '不用。原則只有一個：底部有縫隙或凸起、能讓水流走。最便宜的竹編盤如果結構對，比一個漂亮的陶瓷平盤好用十倍。',
          },
          {
            q: '我家浴室沒窗、超潮濕，皂能放嗎？',
            a: '可以但有兩個調整：一、選矽膠或磁吸式（避免木盤發霉）；二、洗完隨手甩水後拿到客廳通風處，每週至少完整乾燥一次。',
          },
          {
            q: '冷製皂能放冰箱嗎？',
            a: '不需要。冷製皂在乾燥環境下放一年都沒事 ── 反而越熟成越好用。冰箱濕氣其實不低，不一定比抽屜好。',
          },
          {
            q: '皂上長黃斑或白點是壞了嗎？',
            a: '看狀況。少量「DOS（dreaded orange spots）」是油脂氧化的點，可以削掉繼續用、或停用看心情；白色霜狀（蘇打灰）是表面 pH 自然反應，安全可洗。香味變酸臭就是真的壞了。',
          },
          {
            q: '我可以一次拆 6 塊一起放浴室嗎？',
            a: '不建議。每塊都接觸潮氣會同步老化、香氣同步散失。原則：用一塊、拆一塊。其他 5 塊放在乾燥的櫃子裡繼續熟成，越放越好用。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-01',
    cover: {
      src: '/images/journal/humid-island-feet.png',
      alt: '工坊木桌上一塊皂放在編織竹蓆上、旁邊一塊薄棉巾、一只盛著茶樹葉與香茅的木碗、一塊光滑浮石',
      fallback: { tone: 'cool', flower: 'mint' },
    },
    description:
      '台灣濕熱氣候下足部問題很常見 ── 香港腳、足汗、腳臭。本篇講足部皮膚的特殊性、悶熱環境的化學反應、選皂跟日常做法。同時尊重私事的含蓄寫法。',
    keywords: ['香港腳', '足部保養', '台灣濕熱', '足汗', '茶樹皂', '足部清潔'],
    related: ['after-sweat', 'taiwan-water', 'allergy-flare'],
    body: [
      '夏天的台灣，鞋子裡的小宇宙是潮濕的、溫熱的、不通風的 ── 對足部來說，是徹底的逆境。',
      '我們不寫「治療」這件事 ── 那是醫生的領域。香港腳是黴菌感染、屬醫療範疇，本舍的皂是日常清潔輔助、不取代藥物治療。若有典型症狀（趾縫脫皮、水泡、紅腫），請務必先就醫。腳跟皂的關係，是日常生活裡可以做點什麼的範圍。',
      { type: 'h2', text: '足部皮膚的特殊性' },
      '腳掌跟手掌一樣，沒有皮脂腺。皮膚保濕全靠角質層自帶的水分跟外擦的乳液。',
      '腳趾縫之間是台灣濕熱氣候下最容易出問題的地方 ── 皮膚薄、不見光、長期接觸汗水跟襪子纖維。真菌（最常見的紅色毛癬菌）特別愛這個環境。',
      '足汗跟腳臭的成因：汗水本身無味，但腳上的細菌把汗中的胺基酸代謝成短鏈脂肪酸 ── 那個味道。鞋子裡濕熱不通風，細菌增殖速度比皮膚正常更新快。',
      {
        type: 'figure',
        src: '/images/journal/inline/humid-island-feet-1.png',
        alt: '一隻腳側面輪廓手繪示意 ── 周圍有通風的氣流線、旁邊一個放大圈裡是茶樹葉跟一塊浮石',
        caption: '腳的兩個必要條件 ── 透氣與物理代謝',
      },
      { type: 'h2', text: '日常做法' },
      '每天洗腳 + 趾縫擦乾。洗澡時別忽略腳趾縫，洗完用毛巾單獨把每個趾縫擦乾 ── 這個動作比擦身體還重要，但很多人忘記。',
      '襪子一日一換，棉質為主。化纖襪保濕鎖水（不通風）、棉襪吸濕排汗。夏天 2 雙鞋輪流穿，讓每雙穿完後有 24 小時通風時間。',
      '鞋子撒爽身粉或天然吸濕粉（綠豆粉也可）。一週一次，吸掉鞋墊累積的濕氣。',
      '一週一次磨腳。浮石或磨腳石把腳跟厚繭磨薄一點 ── 厚繭裡藏的菌比皮膚表面多。',
      { type: 'h2', text: '選皂選什麼' },
      '一皂到底·清爽款 ── 配方裡有清涼感（薄荷、迷迭香類），夏天洗腳特別舒服。',
      '綠豆清芳·澡豆 ── 綠豆粉有溫和去角質的能力，每週一次當「腳的代謝皂」。',
      '蝶語清瑩·蝶豆 ── 洗感清透不黏，足汗特別重的人，夏天拿來洗腳比厚重保濕款舒服。',
      '不適合：保濕厚重款（霧蜜、桂花）。腳的問題不是缺油，是過油＋過濕。',
      { type: 'h2', text: '什麼狀況該看醫生' },
      '腳趾縫脫皮、長水泡、紅腫 ── 香港腳的典型表現，需要抗黴菌藥膏，不是皂能解決。',
      '腳指甲變黃變厚 ── 灰指甲（甲癬），需要看皮膚科。',
      '腳痛到走路不方便 ── 不是清潔問題了。',
      '皂在足部的角色是「日常維護」── 它讓你的腳保持在不出問題的狀態。出了問題之後，醫生才是主角。',
      {
        type: 'faq',
        items: [
          {
            q: '香港腳治好了還能用本舍的皂嗎？',
            a: '可以而且建議。治好之後最怕復發，每天用清爽款洗腳 + 擦乾趾縫的習慣可以把復發率壓到很低。',
          },
          {
            q: '小孩有腳臭也適用這些做法嗎？',
            a: '原則一樣（換棉襪、洗腳擦乾）。皂可以用大米或一皂到底·清爽 ── 小孩皮膚還薄，先用最溫和的款。',
          },
          {
            q: '泡腳對腳好嗎？',
            a: '溫水泡腳對血液循環跟放鬆有幫助。但泡完一定要擦乾趾縫 ── 否則泡完反而濕氣加倍，容易惡化原本的足部問題。',
          },
          {
            q: '夏天夜市鞋脫一下被味道燻 ── 不只我吧？',
            a: '不只你 ── 這是台灣夏天的集體經驗。解法：當天回家別偷懶，鞋墊抽出來通風、襪子立刻洗、腳用清爽皂洗一次。三件事做齊，明天還是會聞到但會輕一半。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-02',
    cover: {
      src: '/images/journal/allergy-flare.png',
      alt: '工坊木桌上一塊極淡無香的皂躺在白色棉布上，周圍刻意留白 ── 旁邊一只空的素陶碗、一片新鮮蘆薈',
      fallback: { tone: 'cool', flower: 'mint' },
    },
    description:
      '過敏期皮膚需要的是減法 ── 不是加更多東西。本篇講過敏發作期間能不能洗澡、用什麼皂、怎麼降低刺激累積，跟什麼狀況該立刻看醫生。',
    keywords: ['過敏皮膚', '紅疹', '皮膚屏障', '低敏皂', '極簡保養', '敏感期'],
    related: ['skin-ph-acid-mantle', 'baby-soap', 'housewife-eczema'],
    body: [
      '過敏發作的那幾天，皮膚會傳遞很清楚的訊息 ── 不要管我、別碰我、給我空間。',
      '但生活還是要繼續。臉還是要洗、澡還是要沖。問題變成：怎麼在不打擾肌膚回穩的前提下，做最少必要的事？',
      { type: 'h2', text: '過敏期皮膚在做什麼' },
      '過敏發作（不論是接觸性、食物性、季節性）會讓皮膚屏障的緊密接合鬆動 ── 微小分子可以穿過、神經末梢更易被觸發。表現是紅、癢、腫、可能脫屑或滲水。',
      '這個時期每多接觸一個外來物（香精、酒精、收斂劑），都可能變成「壓垮駱駝的最後一根稻草」── 屏障已經負重，再加東西就會崩。',
      {
        type: 'figure',
        src: '/images/journal/inline/allergy-flare-1.png',
        alt: '三個面板的時間進程手繪示意 ── Day 1 紅疹密布、Day 3 紅退伴一塊皂在旁、Day 7 平靜',
        caption: '過敏期的三個時間點 ── 減法是最快的回穩',
      },
      { type: 'h2', text: '減法原則' },
      '保養品數量先砍掉 80%。化妝水、精華、面膜、酸類、A 醇 ── 這幾天全停。只留：洗 + 一瓶單純保濕。',
      '皂選最低敏的款。本舍最常被推薦的是稻花暖心（大米皂無香 + 配方溫和）。如果連大米都覺得「有感」，臨時改用茉莉沐膚（弱酸皂餅 pH 5–6）撐過急性期。',
      '溫水快洗。水溫 35–37°C（接近體溫）、洗澡時間 3–5 分鐘、不需要每天洗（隔天洗一次也行）。',
      '紅腫處不要刻意搓泡 ── 整片皂泡輕輕滑過去就好。「洗乾淨」這三個字過敏期不是優先。',
      { type: 'h2', text: '什麼時候該看醫生' },
      '滲水滲液 ── 屏障已破損、可能有感染風險。',
      '腫脹明顯（眼周、嘴唇、喉嚨） ── 急性過敏反應，可能進展到呼吸道。',
      '癢到睡不著、影響工作、影響情緒 ── 不是「忍一忍就過去」的事，皮膚科有藥可以幫你縮短病程。',
      '連續 7 天用最溫和的方式都沒緩解 ── 不是普通過敏，可能是接觸性皮膚炎或濕疹，要找原因。',
      { type: 'h2', text: '哪些東西是常見的「最後一根稻草」' },
      '香精類保養品（特別是含酒精萃取香料） ── 過敏期完全停。',
      '果酸 / 杏仁酸 / A 醇 ── 即使你之前用得很好，過敏期都先停。',
      '酒精消毒手液 ── 連帶碰到臉會刺激。',
      '羊毛、化纖衣物的直接摩擦 ── 換成純棉。',
      '辛辣食物、酒精、咖啡 ── 內在加溫會放大外在敏感。',
      '過敏期是身體在求救。聽它一次，給它幾天 ── 然後再回去日常。',
      {
        type: 'faq',
        items: [
          {
            q: '過敏期可以完全不洗澡嗎？',
            a: '不建議完全不洗 ── 汗液鹽分本身會刺激敏感皮膚。但可以「溫水沖、不用皂」洗 1–2 天，之後改最溫和的皂（大米）洗 3 分鐘內。',
          },
          {
            q: '過敏發作擦類固醇藥膏可以同時用本舍的皂嗎？',
            a: '可以。皂洗完徹底沖掉、皮膚乾後再擦藥膏。皂跟藥膏不衝突 ── 皂負責清潔、藥膏負責治療。但若皮膚科醫生有特別交代「停皂」就照他的。',
          },
          {
            q: '怎麼判斷是真的對皂過敏還是其他原因？',
            a: '停皂 7 天看狀況。如果停皂後緩解 → 可能對皂裡某成分敏感（換成大米無香試）。如果停皂仍持續 → 不是皂的問題，找其他原因（食物、新買的衣物、環境）。',
          },
          {
            q: '小孩過敏可以套用同樣原則嗎？',
            a: '原則一樣（減法）但不要自己換藥。小兒過敏建議直接看小兒科或皮膚科 ── 小孩的皮膚跟成人不同節律，亂用大人的處理方式可能延誤。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-03',
    cover: {
      src: '/images/journal/menstrual-skin.png',
      alt: '工坊木桌上一塊皂躺在大地色的素麻巾上，旁邊一只飄著熱氣的玫瑰果茶杯、一枚乾燥玫瑰花瓣、一顆深灰光滑的河石',
      fallback: { tone: 'warm', flower: 'rose' },
    },
    description:
      '月經週期 28 天裡，雌激素跟黃體素的起伏直接影響皮脂分泌、敏感度跟痘痘 ── 同樣的皂在不同週洗起來感受不一樣。本篇講四個階段的皮膚特性跟對應選皂。',
    keywords: ['經期皮膚', '生理期保養', '荷爾蒙與皮膚', '經前痘痘', '女性週期', 'PMS'],
    related: ['skin-ph-acid-mantle', 'how-to-choose-soap', 'menopause-skin'],
    body: [
      '一個讀者問：「為什麼我月經前一週洗你們的桂花皂，洗完整張臉是緊繃的？平常洗都很舒服啊。」',
      '不是桂花皂的問題。她那一週的皮膚，跟其他三週不是同一張皮膚。',
      { type: 'h2', text: '28 天裡的四種皮膚' },
      '月經週期裡，雌激素跟黃體素像兩個輪流上場的演員 ── 一個讓皮膚保水、有光澤、毛孔細；另一個讓皮脂分泌增加、毛孔擴張、容易敏感。',
      '經期（第 1–5 天）── 雌激素最低點。皮膚容易乾、敏感、暗沉。最不適合做角質代謝、最不適合用清潔力強的皂。',
      '濾泡期（第 6–14 天）── 雌激素穩定上升、皮膚最好的時候。光澤、彈性、毛孔細 ── 自拍最美在這幾天不是錯覺。',
      '排卵期（第 14–17 天）── 雌激素達高峰，黃體素開始上升。皮膚仍漂亮但開始悄悄出油。',
      '黃體期（第 17–28 天）── 黃體素主導。皮脂分泌增多、容易冒痘（特別是下巴與下顎線）、屏障敏感度上升。',
      {
        type: 'figure',
        src: '/images/journal/inline/menstrual-skin-1.png',
        alt: '一個 28 天的圓形月曆手繪示意 ── 四個象限分別標示清淨、油性、敏感、平穩等皮膚狀態',
        caption: '一個月的四種皮膚 ── 同一張臉，狀況不一樣',
      },
      { type: 'h2', text: '對應的選皂節奏' },
      '經期：選最溫和的款 ── 大米、海棠、霧蜜。洗臉時間縮短到 30 秒、水溫降一級、別追求「洗得很乾淨」。',
      '濾泡期：彈性最大 ── 大概用什麼皂都沒問題。可以試新款。',
      '排卵到黃體期：往清潔力多一點移 ── 綠豆、蝶豆、一皂到底·清爽。下顎線冒痘可以晚上多洗一次（早上用桂花、晚上用綠豆）。',
      'PMS（經前症候群）兩天：很多人會睡眠不足、心情緊。慢洗澡 + 桂花潤膚 + 一杯熱茶 ── 不是直接解 PMS，是給身體一個被照顧的儀式。',
      { type: 'h2', text: '一個小提醒' },
      '記錄三個月看自己的節奏。每個女生的週期不一樣 ── 28 天是中位數，有人 24 天有人 32 天。記下「哪週皮膚特別好、哪週特別敏感」── 你會找到自己的選皂節奏。',
      '皮膚的節律是身體在跟你說話。聽得到，照顧才會準。',
      {
        type: 'faq',
        items: [
          {
            q: '經期皮膚特別敏感，連洗澡水都覺得刺，怎麼辦？',
            a: '降水溫到 36–37°C（接近體溫）、洗澡時間 5 分鐘內、皂選大米或弱酸皂餅（茉莉沐膚）。一週後恢復就好，不需要長期換。',
          },
          {
            q: '經前長下顎痘可以擠嗎？',
            a: '不建議。荷爾蒙性痘的成因在內，擠了還是會再冒。可以做的：那一週清潔加強、保養減量（少擦乳液避免悶）、紅腫處冰敷。',
          },
          {
            q: '吃避孕藥會影響皮膚嗎？',
            a: '會。複方避孕藥讓荷爾蒙波動變平穩，多數人皮膚會變穩定（但對少數人反向）── 跟皮膚科醫生討論個人情況。皂的選擇沒有差，順著當下狀態挑。',
          },
          {
            q: '更年期之後是不是就沒有這些週期變化？',
            a: '是的。停經後雌激素穩定低，皮膚進入另一種狀態（往乾性化走）── 詳細在〈更年期的皮膚〉那篇。週期波動結束，但新的挑戰開始。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-04',
    cover: {
      src: '/images/journal/housewife-eczema.png',
      alt: '工坊木桌上一塊皂放在折好的廚房棉巾旁，旁有微髒的圍裙、一只敞口的蜂蠟護手膏陶罐、一束新鮮檸檬香蜂草',
      fallback: { tone: 'cool', flower: 'mugwort' },
    },
    description:
      '主婦濕疹（手部濕疹）跟反覆接觸水跟洗劑直接相關。每洗一次手皮膚屏障被剝一層 ── 一天 20 次累積起來就是危機。本篇講手部皮膚的限度、選皂選洗碗精的原則、跟回穩節奏。',
    keywords: ['主婦濕疹', '手部濕疹', '反覆洗手', '家事手', '護手皂', '手部保養'],
    related: ['skin-ph-acid-mantle', 'allergy-flare', 'soap-storage'],
    body: [
      '一個媽媽朋友手指龜裂滲血給我看 ── 她說「我每天洗手不下二十次，是不是水太硬？」',
      '不是水。是「次數」這件事本身。',
      { type: 'h2', text: '為什麼手會壞' },
      '手部皮膚跟臉不一樣 ── 手沒有皮脂腺（手掌完全沒有、手背很少）。臉的皮脂膜可以靠分泌補回來，手沒有這個機制。',
      '每洗一次手，表層的天然脂質被洗掉一點。一天 20 次洗手 + 5 次洗碗 + 3 次酒精消毒 + 兩次洗澡 ── 手部皮膚一天經歷 30 次清潔事件。沒有任何屏障撐得住。',
      {
        type: 'figure',
        src: '/images/journal/inline/housewife-eczema-1.png',
        alt: '一條向下掉的手繪曲線示意 ── 隨著洗手次數增加，皮膚水分一直下降，最低點放著一塊皂作為救援',
        caption: '一天的洗手次數 vs 屏障狀態 ── 累積起來看才會嚇到',
      },
      { type: 'h2', text: '解決的兩條軸線' },
      '減損：把每次清潔的傷害降到最小。',
      '回穩：每天有時間讓皮膚自己補回來。',
      { type: 'h2', text: '減損 ── 選對洗手皂' },
      '冷製手工皂洗手比工業洗手乳溫和很多 ── 因為甘油保留下來，洗完有薄薄一層水分。本舍最常被選來洗手的是稻花暖心（大米皂無香）跟瓊崖海棠（海棠潤澤皂）。',
      '洗碗用什麼？這也是日常。市售洗碗精含 SLS（高清潔強度）洗手會把皮膚屏障洗光。改用：洗碗時戴橡膠手套 + 用本舍的綠豆清芳當洗碗皂（綠豆粉幫去油又溫和）。聽起來「土」，但雙手到 60 歲還能用。',
      '酒精消毒：盡量改成洗手代替。COVID 之後很多人習慣酒精消毒手 ── 酒精把皮膚脂質溶光的速度比皂快 5 倍。除非真的接觸到髒污，正確洗手 30 秒就夠。',
      { type: 'h2', text: '回穩 ── 給手時間' },
      '洗完手立刻擦護手乳。3 分鐘內最有效 ── 沖水後手部皮膚是「半濕」狀態，這時擦的乳液鎖水率最高。',
      '晚上睡前的「手部潤夜」── 洗完雙手用蜂蠟膏（或厚一點的潤膚膏）厚塗，戴一雙純棉薄手套睡覺。一週兩次，龜裂的指縫一個月內會慢慢平緩。',
      '濕疹發作期不要用任何含香精的東西。包括有香味的皂 ── 過敏可能性高。改用大米皂（無香）撐過去，等急性期穩定再回到日常款。',
      '一雙手是一輩子的事。它們做的事很多人看不到，但它們知道。',
      {
        type: 'faq',
        items: [
          {
            q: '我手已經有濕疹了，還能用冷製皂嗎？',
            a: '急性發作期建議先用無香的款（大米）或弱酸皂餅（茉莉沐膚）；穩定期可以回到瓊崖海棠這類潤澤款。如果濕疹嚴重請看皮膚科 ── 皂是日常輔助，類固醇藥膏才是急性處理。',
          },
          {
            q: '洗碗用海棠這類「貴的皂」會不會浪費？',
            a: '不會 ── 一塊用三個月，比起反覆濕疹看皮膚科的成本便宜得多。如果還是覺得心疼，洗碗用綠豆清芳（價位適中、清潔強度夠）就好。',
          },
          {
            q: '常用酒精的工作（醫護、餐飲）怎麼辦？',
            a: '工作期間沒辦法停酒精。但每次酒精乾後立刻擦薄乳液（口袋放一管 30 g 護手乳），下班後用最溫和的款（大米、海棠）洗手 + 厚塗護手霜睡覺。傷害無法清零，但能拉回平衡。',
          },
          {
            q: '小孩家事一起做，手會不會也壞？',
            a: '小孩的肌膚回穩能力比成人強 ── 但偶發性的家事（每週一次）不會出問題；天天洗碗反覆接觸洗碗精會。同樣建議：戴手套或用更溫和的洗劑。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-05',
    cover: {
      src: '/images/journal/elderly-winter-itch.png',
      alt: '工坊木桌上一塊皂躺在折好的羊毛毯上，旁邊一只冒著蒸氣的陶壺、一小盤乾燥金盞花瓣、一雙穿舊的羊毛拖鞋',
      fallback: { tone: 'warm', flower: 'osmanthus' },
    },
    description:
      '七十歲後皮脂腺分泌量是年輕時的一半，冬天乾冷加上洗澡水太燙就會搔癢 ── 不是疾病，是皮膚乾性化的訊號。本篇講老人皮膚的變化、冬天洗澡的調整、跟適合長輩的皂款。',
    keywords: ['老人乾癢', '冬季搔癢', '長者皮膚', '銀髮族保養', '冬季皂選', '保濕皂'],
    related: ['menopause-skin', 'skin-ph-acid-mantle', 'how-to-choose-soap'],
    body: [
      '冬天回老家，最常聽到的句子之一是「我這小腿最近一直癢」。年輕人會以為是過敏、感染、什麼皮膚病 ── 多數時候不是。是「老人乾癢」。',
      '七十歲後皮脂腺分泌量是 30 歲時的一半。皮膚自帶的油脂少了，外加冬天乾冷低濕度、洗澡水偏燙、室內暖氣 ── 四件事疊起來，皮膚屏障就被掏空了。',
      { type: 'h2', text: '老人乾癢的三個訊號' },
      '小腿、前臂、手肘、後背 ── 這四個地方最早出現。皮膚看起來有細白皮屑、用手摸有粗砂感、晚上睡覺前特別癢。',
      '不是過敏。是皮脂膜薄到無法鎖住水分，神經末梢直接接觸到外界刺激，大腦就讀成「癢」。',
      { type: 'h2', text: '為什麼冬天特別嚴重' },
      '冬天三件事疊加：',
      '一、空氣濕度低（冷空氣鎖不住水氣）── 皮膚水分一直蒸發。',
      '二、暖氣 / 電熱器 / 電毯讓室內更乾。',
      '三、長輩冬天偏好洗熱水澡（「暖暖才舒服」）── 但 42°C 以上的水會把表皮油脂洗光。',
      '解法不是不洗澡 ── 是把這三件事各自調整一點。',
      {
        type: 'figure',
        src: '/images/journal/inline/elderly-winter-itch-1.png',
        alt: '冬日場景手繪示意 ── 左邊一棵光禿的樹、右邊一塊乾裂皮膚的特寫圖樣，中間擺著一塊皂',
        caption: '冬與乾癢之間 ── 一塊皂的位置',
      },
      { type: 'h2', text: '冬天洗澡的調整' },
      '水溫降一級。從 42°C（很燙）降到 38°C（溫熱）── 體感差別不大，對皮膚差很多。',
      '頻率不要追求每天。長輩冬天每天洗澡反而會把油洗光。隔天洗一次、其他日子用熱毛巾擦擦也夠。',
      '時間 5 分鐘內。比起夏天的 10–15 分鐘，冬天 5 分鐘洗完起來最舒服 ── 多 5 分鐘是熱水浪費 + 皮膚多被洗一遍。',
      '洗完立刻擦乳液 / 乳霜。別等 ── 老人皮膚的水分流失速度比年輕人快兩倍。沖水後 3 分鐘內擦上保濕，鎖水效率最高。',
      { type: 'h2', text: '長輩適合的皂款' },
      '槐花蜜潤·霧蜜 ── 霧峰蜂蜜潤厚、配方為冬天設計、洗完不緊。長輩冬天最不會挑剔的款。',
      '杜康醉月·酒粕 ── 酒粕的滋潤跟米的甘甜，洗起來像被一層厚毯包住。',
      '桂月流金·桂花 ── 桂花的甜香長輩特別喜歡（喚起記憶），配方溫潤滋養。',
      '不適合：清爽款（油性肌用的）、洗髮餅以外的高清潔款 ── 皮脂本來就少，再洗就見底了。',
      '送長輩冬季禮，這三款選兩塊送 ── 比一張保暖貼布實用得多。',
      {
        type: 'faq',
        items: [
          {
            q: '長輩說洗完癢更嚴重，是皂的關係嗎？',
            a: '可能是水太燙、皂選太清潔 ── 兩個都改一下試試。如果換到霧蜜或桂花後仍癢，去看皮膚科 ── 老年搔癢有時跟肝腎功能、糖尿病有關，不是皂能解決的。',
          },
          {
            q: '冬天可以洗熱水澡嗎？',
            a: '可以，但水溫 38–40°C 內。「熱水裡泡很久」對長輩不只皮膚的事 ── 心血管也吃不消。建議短洗澡 + 之後喝杯熱湯比泡熱水浴安全。',
          },
          {
            q: '長輩討厭擦乳液，怎麼辦？',
            a: '改成「皂裡多一點滋潤」── 霧蜜這類本身就帶豐富潤膚成分的皂，洗完已經有薄薄一層保濕。雖然不能完全取代乳液，但對抗拒擦乳液的長輩是務實妥協。',
          },
          {
            q: '老人皮膚薄擦乳液會吸收嗎？',
            a: '會，而且比年輕人吸收得更快（屏障變薄）── 這正是擦乳液對長輩特別重要的理由。選擇沒有強香料、無酒精的單純保濕乳就好。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-06',
    cover: {
      src: '/images/journal/teen-acne.png',
      alt: '工坊木桌上一塊淡綠色皂塊放在大理石紋面盆邊，旁邊一條折好的白色面巾與一碗鮮茶樹葉',
      fallback: { tone: 'cool', flower: 'mugwort' },
    },
    description:
      '青春期洗臉最常見的兩個錯誤：洗太多次反而出更多油、洗太用力把屏障洗壞了。本篇講皮脂腺在青春期的活動、洗臉的合理頻率、跟適合的皂款。',
    keywords: ['青少年痘痘', '青春期洗臉', '油性肌', '控油皂', '茶樹皂', '高中生保養'],
    related: ['skin-ph-acid-mantle', 'how-to-choose-soap', 'allergy-flare'],
    body: [
      '老闆娘鍋邊有兩個高中生客人 ── 都是來買皂給自己的。一個說「我每天洗臉七次還是滿臉油」，另一個說「我都不敢洗怕越洗越乾」。',
      '兩個都錯。中間有一條線，需要找到。',
      { type: 'h2', text: '為什麼青春期會冒痘' },
      '青春期皮脂腺受雄激素影響開始大量分泌油脂 ── 男生女生都會，男生尤其。皮脂分泌量在 14–18 歲達到高峰，之後 5–10 年慢慢下降。',
      '油本身不是壞 ── 它是皮膚屏障的一部分。會冒痘是因為三件事同時發生：油多 + 角質代謝太快堵住毛孔 + 痤瘡桿菌（C. acnes）在堵住的毛孔裡造成紅腫。三個條件中斷任一個，痘痘就會減少。',
      {
        type: 'figure',
        src: '/images/journal/inline/teen-acne-1.png',
        alt: '毛囊剖面手繪示意 ── 三個狀態並排：清通、油堵、紅腫',
        caption: '一個毛囊的三段時間 ── 從通順、被堵、到紅腫',
      },
      { type: 'h2', text: '洗太多反而冒更多' },
      '一天洗臉七次會發生什麼：每次洗都把表層油洗掉、皮脂腺接到「油不夠」的訊號於是分泌更多、半小時後又油 ── 進入「越洗越油」的循環。',
      '正解：早晚各一次。中間如果汗很多、運動完，用清水沖一下就好，不要每次都用皂。',
      '另一個錯誤：用很燙的水洗。覺得「熱水殺菌」「熱水溶油」── 都是錯覺。熱水會把脂肪膜整個破壞，洗完皮膚雖然「滑」但屏障已經損傷，痘痘反而更難收。溫水（35–38°C）洗臉最對。',
      { type: 'h2', text: '青春期適合的皂款' },
      '綠豆清芳·澡豆 ── 研磨綠豆粉幫角質代謝、配方溫和不刮。這是本舍給青少年最常推薦的款，痘痘困擾連用一個月會有感。',
      '蝶語清瑩·蝶豆 ── 洗感比綠豆再清爽一點，喜歡清透收乾感的人可以換著用。',
      '一皂到底·清爽款 ── 男高中生最容易接受的款。一塊全身能洗、配方裡薄荷類精油給清涼感、洗完不油。比起一堆瓶瓶罐罐簡單多。',
      '不適合：純橄欖、霧蜜、桂花這些保濕厚實款 ── 油性肌用會悶。等青春期之後皮脂分泌穩定再考慮。',
      { type: 'h2', text: '日常的小事' },
      '不要擠痘痘。手上的細菌會讓本來會自己消的痘變成留疤的痘。'
,
      '枕頭套一週洗一次。臉貼著一週的枕頭睡七晚 ── 上面累積的皮脂、塵蟎、頭油都進到毛孔。換洗頻率是最便宜的痘痘管理。',
      '飲食的關係：高 GI 食物（含糖飲料、白麵包、白米）跟乳製品在大量研究中跟青少年痘痘有關聯。不是要禁，是少一點。',
      '青春期會過去。一塊溫和的皂、不要洗太勤、別擠痘痘 ── 這三件事做對，渡過去之後皮膚會穩。',
      {
        type: 'faq',
        items: [
          {
            q: '一天到底洗幾次臉？',
            a: '基本：早一次、晚一次。例外：運動或大量出汗後用清水沖（不用皂）。最大上限：三次（含清水沖）。超過就是過度清潔。',
          },
          {
            q: '冷製皂的鹼性會不會讓痘痘更嚴重？',
            a: '不會 ── 健康年輕皮膚 30 分鐘恢復弱酸沒問題。痘痘的成因是油脂跟毛孔紅腫，不是 pH。冷製皂選對款（綠豆、蝶豆）反而幫助。',
          },
          {
            q: '我同學都用皮膚科的洗面乳，我用手工皂會輸嗎？',
            a: '不會。皮膚科洗面乳走的是合成界面活性劑路線（控油較強），冷製皂走的是溫和清潔路線。各有優勢。如果你已經在皮膚科治療痘痘，皂是補充不是替代 ── 醫師建議優先。',
          },
          {
            q: '皂用完痘痘還是冒，要不要換？',
            a: '一塊皂連用 4 週才看得出效果。如果 4 週後仍嚴重，先看皮膚科 ── 痘痘有時是需要藥物治療的（特別是會留疤的那種），皂在這個階段是輔助不是主力。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-07',
    cover: {
      src: '/images/journal/menopause-skin.png',
      alt: '工坊木桌上一塊皂躺在折好的素絹布上，旁邊一面銅鏡反扣、一束乾燥鼠尾草、一支半燒的蜜蠟燭',
      fallback: { tone: 'warm', flower: 'osmanthus' },
    },
    description:
      '更年期皮膚會變薄、變乾、變敏感 ── 雌激素下降直接影響皮脂分泌跟膠原蛋白合成。為什麼以前用得好的皂突然不行了？要怎麼換？本篇講荷爾蒙變化下的選皂跟生活調整。',
    keywords: ['更年期皮膚', '熟齡保養', '皮膚變薄', '荷爾蒙與皮膚', '敏感肌轉換', '50 歲'],
    related: ['skin-ph-acid-mantle', 'how-to-choose-soap', 'menstrual-skin'],
    body: [
      '老闆娘鍋邊收到一封五十二歲讀者的來信：「我用了你們海棠皂八年了一直很愛，這半年突然洗完臉開始癢、開始緊。是你們配方變了嗎？」',
      '配方沒變。她變了。',
      '更年期不是一個事件，是一段過程 ── 通常從 45 歲左右開始，到 55 歲前後完成。雌激素逐漸下降，皮膚是這場變化最早被看見的地方。',
      { type: 'h2', text: '皮膚在更年期會發生什麼' },
      '雌激素下降直接影響三件事：皮脂分泌減少（變乾）、膠原蛋白合成下降（變薄）、皮膚屏障恢復力變慢（變敏感）。',
      '具體到日常：洗完臉緊繃感比以前強、保濕乳液要用比以前多、有些以前不會泛紅的東西開始泛紅、皮膚厚度可能在 5 年內掉 20%。',
      '這不是壞 ── 這是身體在做必要的轉換。但日常用品不調整就會出狀況。',
      {
        type: 'figure',
        src: '/images/journal/inline/menopause-skin-1.png',
        alt: '三個皮膚剖面手繪示意，從 30 歲、50 歲到 70 歲 ── 表皮層逐漸變薄、皮脂腺逐漸縮小',
        caption: '皮膚的時間軸 ── 雌激素退場後，每一層都變薄一點',
      },
      { type: 'h2', text: '為什麼以前的皂突然不行' },
      '冷製皂本身是鹼性（pH 8–10）── 健康皮膚 30–60 分鐘可以拉回弱酸。但更年期皮膚的「拉回」變慢了 ── 可能要 90 分鐘、120 分鐘。',
      '中間這段時間，皮膚屏障處在最脆弱狀態。如果你接著做了「擦化妝水→精華→乳液→眼霜」一系列保養動作，每一步都可能被薄屏障接收成「刺激」── 結果是越保養越敏感。',
      '解法不是「停用冷製皂」── 是「用更溫和的款」加上「換洗順序」。',
      { type: 'h2', text: '熟齡選皂的兩個原則' },
      '原則一：往「保濕厚實」那一端移。如果以前用海棠（中度潤澤），現在可以選霧蜜（蜂蜜潤厚）；以前用大米（清爽溫和），現在選桂花潤膚或杜康醉月（酒粕滋潤）。',
      '原則二：臉跟身體分開。臉用弱酸皂餅（茉莉沐膚 pH 5–6），洗完不需要等屏障拉回；身體繼續用冷製皂沒問題（身體角質厚、恢復力仍夠）。這是更年期最重要的轉換。',
      { type: 'h2', text: '生活的微調' },
      '水溫降下來。更年期之前 40°C 的熱水洗澡舒服，更年期之後同樣溫度可能讓皮膚刺癢。降到 36–38°C ── 接近體溫的水最不打擾屏障。',
      '洗澡時間縮短。10 分鐘內洗完比 20 分鐘更友善。慢澡是好事但留給特定夜晚（不是天天）。',
      '保養品從「多層」改「一層厚的」。一瓶質地好的保濕乳取代化妝水＋精華＋乳液三步驟 ── 步驟越少對薄屏障越友善。',
      '這段路不必走得辛苦。一塊溫和的皂、一個簡化的保養流程、一段對自己的耐心 ── 皮膚會找到新的穩定。',
      {
        type: 'faq',
        items: [
          {
            q: '我才 42 歲，皮膚已經開始變敏感算更年期嗎？',
            a: '可能是「圍更年期」（perimenopause）── 真正更年期前的 5–10 年荷爾蒙波動就已經開始。應對方式跟更年期一樣，不必等診斷確認再調整保養。',
          },
          {
            q: '聽說更年期不能用冷製皂只能用沐浴乳，是真的嗎？',
            a: '不真。冷製皂不是更年期的敵人 ── 過度刺激的清潔劑（含 SLS、強香精）才是。配方溫和的冷製皂（霧蜜、桂花、海棠）洗身體完全沒問題，臉換弱酸皂餅就好。',
          },
          {
            q: '皮膚突然變很乾，有沒有救？',
            a: '皮膚的乾不是不可逆 ── 換對皂、降水溫、減少保養層數，2–3 個月會穩定下來。不要急著一次換全部產品，從一塊皂開始。',
          },
          {
            q: '更年期送禮給同齡朋友，選哪一塊？',
            a: '桂月流金（桂花潤膚）── 香氣甜而長、配方溫潤、外觀斯文。是熟齡同輩最不踩雷的禮。要更厚實一檔可以選霧蜜（霧峰蜂蜜）。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-13',
    cover: {
      src: '/images/journal/baby-soap.png',
      alt: '工坊木桌上一塊小巧的乳白色皂躺在棉布上，旁邊有一支木製安撫圈、一朵乾燥洋甘菊與一小瓶羊乳',
      fallback: { tone: 'warm', flower: 'osmanthus' },
    },
    description:
      '寶寶能不能用冷製手工皂？媽媽哺乳期皮膚怎麼選皂？老闆娘鍋邊收到最多的問題之一。本篇講嬰兒皮脂膜的發展階段、產後媽媽的選皂、以及兩個人都用得到的款。',
    keywords: ['嬰兒手工皂', '寶寶洗澡', '產後皮膚', '哺乳期', '敏感肌嬰兒', '冷製皂'],
    related: ['skin-ph-acid-mantle', 'how-to-choose-soap', 'allergy-flare'],
    body: [
      '老闆娘鍋邊收到最多的兩個問題：一個是「我的寶寶能用你們的皂嗎」，另一個是「我手洗到龜裂了，哺乳期能用什麼」。',
      '這兩件事其實是同一個故事的兩面 ── 一個剛出生的寶寶跟一個剛當媽的女人，住在同一個家裡，皮膚同時都在重整。',
      { type: 'h2', text: '嬰兒皮膚跟成人皮膚的差別' },
      '剛出生的寶寶皮膚厚度只有成人的 1/3。皮脂腺還沒完全運作、皮脂膜（acid mantle）還在建立 ── 這層東西是皮膚對外的天然屏障，要到 6 個月之後才慢慢成熟、3 歲之後才接近成人狀態。',
      '所以嬰兒皮膚對任何外來刺激（鹼性、香精、色素、防腐劑）都比成人敏感。一個成人覺得「沒感覺」的清潔劑，可能讓寶寶起紅疹。',
      {
        type: 'figure',
        src: '/images/journal/inline/baby-soap-1.png',
        alt: '兩個皮膚剖面手繪示意 ── 左邊成人皮膚厚實的角質層，右邊嬰兒皮膚薄而疏鬆',
        caption: '同樣是皮膚 ── 嬰兒只有成人的 1/3 厚',
      },
      { type: 'h2', text: '三個階段，三種選擇' },
      '0–1 個月（新生兒期）── 任何皂都先放下。這個階段純清水擦拭就夠，醫護建議的也是溫水＋紗布。我們不會推薦本舍任何一塊冷製皂給新生兒。',
      '1–6 個月（嬰兒期）── 皮脂膜慢慢建立，但還是脆弱。建議走「弱酸皂餅」這條路 ── pH 5–6 接近肌膚原生酸鹼，不打擾屏障建立。本舍的茉莉沐膚（弱酸沐浴餅）配方已經是低敏設計，但寶寶肌膚仍建議先在小範圍試三天再擴大使用。',
      '6 個月以後（皮脂膜逐漸成熟）── 可以考慮溫和的冷製皂。本舍的稻花暖心（大米皂）跟瓊崖海棠潤澤皂是最常被媽媽選的兩款 ── 大米無香、海棠的配方為敏感肌設計。建議仍是先試一塊小範圍、觀察 24 小時，沒紅、沒疹、沒乾再擴大使用。',
      { type: 'h2', text: '紅屁屁、汗疹、奶癬' },
      '這三件事不是用皂能解決的 ── 它們的起因不在「不夠乾淨」，反而是「過度清潔」或「悶熱不通風」造成的。',
      '紅屁屁多半是尿布悶住 + 排泄物的酸鹼刺激。對策是勤換尿布、屁屁洗完用乾淨的紗布輕拍乾、不要再用皂洗第二次。',
      '汗疹是夏天高溫流汗、汗液沒散開造成的。對策是降溫、通風、輕擦，不需要動到皂。',
      '奶癬（脂漏性皮膚炎）成因不明、自限性，多數寶寶 6 個月內自己會好。皮膚科醫師會給你方向，皂在這件事上不會幫上忙也不會壞事。',
      '簡單說：皂是清潔，不是治療。寶寶皮膚有任何問題請先看小兒科或皮膚科 ── 醫師判斷之後，皂才會回到「日常清潔」這個位置。',
      { type: 'h2', text: '哺乳期媽媽自己呢' },
      '產後跟哺乳期的媽媽，皮膚會經歷三件事 ── 荷爾蒙波動讓皮脂分泌不規律（時油時乾）、頻繁洗手洗到角質流失、抱寶寶睡眠不夠導致敏感。',
      '這個階段最好用的兩款 ── 瓊崖海棠潤澤皂（用全身 + 洗臉）跟山茶淨髮髮餅（洗頭髮）。前者厚實潤澤、後者是 pH 5–6 對 24 小時都接觸寶寶的敏感頭皮溫和。',
      '另一個常被忘記的：擠奶 / 哺乳前洗手用什麼？很多人用酒精、用消毒洗手乳，但這對乳頭周圍皮膚很傷。建議純清水沖洗、或本舍的霧蜜（霧峰蜜潤）洗一下沖乾淨 ── 蜂蜜的溫和度不會把油脂洗光。乳頭本來就有自己的菌群跟油脂保護，過度消毒反而適得其反。',
      '寶寶跟媽媽的皮膚都在重整這件事，沒有捷徑。一塊溫和的皂、一些觀察、必要時看醫生 ── 這個組合是穩當的方式。',
      {
        type: 'faq',
        items: [
          {
            q: '本舍有「嬰兒專用皂」嗎？',
            a: '沒有獨立產品線。我們的判斷是 ── 與其開一支「嬰兒專用」打上行銷標籤，不如告訴媽媽我們十二款裡哪些最適合（大米、海棠），以及什麼階段不該用皂（新生兒期）。',
          },
          {
            q: '聽說有人用冷製皂給寶寶從出生就洗，這樣可以嗎？',
            a: '從育兒醫學角度不建議。新生兒期皮膚屏障未成熟，溫水擦拭已足夠。冷製皂等 6 個月後再考慮。網路上的不同經驗各家有各家的判斷，但本舍給的建議偏保守。',
          },
          {
            q: '哺乳期可以用有香味的皂嗎？',
            a: '可以，但建議避免在哺乳當下用太強烈的香（精油類）── 寶寶會聞到。日常洗澡選溫和甜香（桂花、米香）安全；玫瑰艾草等高揮發性精油款先放著。',
          },
          {
            q: '寶寶試了一塊皂沒紅疹是不是就 OK 了？',
            a: '基本上是。但建議仍是「同一塊用 1–2 週」確認沒有累積性反應之後，才當固定款使用。皮膚的訊號有時會延遲。',
          },
          {
            q: '產後脫髮可以用山茶淨髮餅嗎？',
            a: '可以。產後脫髮成因是荷爾蒙回穩，跟皂的選擇無直接關係 ── 洗髮餅的角色是「不要再增加損傷」。山茶淨髮的 pH 接近頭皮原生酸鹼、無強清潔劑，是哺乳期最不挑剔的選擇。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-04-29',
    cover: {
      src: '/images/journal/scent-and-memory.png',
      alt: '工坊木桌上一塊皂躺在攤開的皮革日記簿頁面之間，旁邊三只精油小玻璃瓶與一張反扣的舊照片',
      fallback: { tone: 'warm', flower: 'osmanthus' },
    },
    description:
      '香氣是嗅覺神經唯一直通海馬迴的感官 ── 比視覺、聽覺更快觸發記憶。本篇講氣味跟記憶的神經科學、本舍十二款的氣味地圖、跟為什麼某些香會讓你哭。',
    keywords: ['香氣記憶', '普魯斯特效應', '嗅覺神經', '精油與情緒', '皂的香氣', '香氛'],
    related: ['slow-bath', 'gift-soap', 'how-to-choose-soap'],
    body: [
      '一個讀者來信：「我用桂花皂洗了第三次，突然想起七歲那年外婆家後院那棵桂花。我以為那段記憶早就被我搞丟了。」',
      '不是搞丟。是收得很深。香氣是把它叫醒的鑰匙。',
      { type: 'h2', text: '香氣的神經捷徑' },
      '視覺、聽覺、觸覺的訊號都要先經過大腦的「視丘」這個中繼站，才會送到記憶相關的海馬迴。',
      '嗅覺是唯一的例外 ── 氣味分子刺激嗅球後，直接連到杏仁核（情緒）跟海馬迴（記憶），不經過視丘。',
      '這個「神經捷徑」是為什麼香氣的記憶觸發比其他感官都快、都直接、都帶情緒。看到一張照片你會「想起來」；聞到一股氣味你會「整個人回到那裡」。',
      {
        type: 'figure',
        src: '/images/journal/inline/scent-and-memory-1.png',
        alt: '一條從鼻子側面剪影出發、彎曲向上連到一個帶光點的腦輪廓的手繪示意 ── 嗅覺直達海馬迴',
        caption: '嗅覺的神經捷徑 ── 從鼻到記憶只有一條短路',
      },
      { type: 'h2', text: '十二款的氣味地圖' },
      '本舍十二款各有香氣，不是為了聞起來「好聞」── 是為了某個情境、某段時間。下面這份地圖可以當參考：',
      '海棠（瓊崖海棠油）── 木質堅果調，乾淨穩重的氣息。讓人想起山林、雨後、深處的木桌。',
      '霧蜜（霧峰蜂蜜）── 蜜的甘潤帶一點淡淡的花果。是冬天溫暖被窩的味道。',
      '綠豆（研磨綠豆粉）── 米穀類的清香、有點豆沙的甜。涼夏午後外婆家的綠豆湯。',
      '蝶豆（蝶豆花）── 花植青草調，乾淨而年輕。像剛拆封的新書頁面。',
      '長金（長濱金盞花）── 草本溫和的金黃感。讓人想起山道上慢慢走的午後。',
      '星米（三星米）── 米的暖香、樸實。家的廚房、剛蒸好的飯。',
      '酒粕（杜康醉月）── 米酒的甜香 + 一點發酵感。某個阿公那杯小酒、過年的桌邊。',
      '桂花（桂月流金）── 蜜甜清遠的花香。九月、家門口、第一陣涼風。',
      '山茶（淨髮髮餅）── 山茶花油溫潤、加蠶絲蛋白的乾淨。剛梳理好的頭髮。',
      '茉莉（沐膚餅）── 茉莉的甘香淡遠。江南某個午後、廟口的午後。',
      '清爽（一皂到底·清爽）── 薄荷迷迭香的清涼。夏天剛沖完澡。',
      '保濕（一皂到底·保濕）── 厚實的木香加一點甜。冬天剛沖完澡。',
      { type: 'h2', text: '為什麼某個香會讓你哭' },
      '香氣連到的是杏仁核 ── 大腦處理情緒（包括恐懼、悲傷、快樂）的地方。一個跟某段強烈情緒一起被存進去的香氣，幾年甚至幾十年後再聞到，杏仁核就會把那段情緒一起召回。',
      '這不只是「想起」── 是「重新感受」。所以你會在一個普通的下午、洗一塊普通的皂、突然眼眶熱了。',
      '這不是壞事。是身體在告訴你 ── 那個人、那個地方、那段時間，原來真的對你那麼重要。',
      { type: 'h2', text: '怎麼選一塊「對的香」' },
      '送禮：選對方有美好聯想的香。問家人「她喜歡桂花嗎」「他外婆是不是常做酒粕料理」── 答案就是禮的方向。',
      '自用：選一個你想留下記憶的香。新搬家的浴室用桂花、剛當媽用大米、剛退休用酒粕 ── 五年後回憶這段時間，香氣會幫你找回來。',
      '撫慰：選一個讓你安心的香。失眠夜選薰衣草（部分清爽款裡有）、需要明亮的時候選蝶豆、想被擁抱的時候選霧蜜。',
      '一塊皂的香，從來不只是香。它是一段時間的把手 ── 你現在用它、未來也許就用它去找回現在。',
      {
        type: 'faq',
        items: [
          {
            q: '我聞不太到本舍的皂香，是不是配方放太少？',
            a: '冷製皂的香比工業香水淡 8 倍 ── 因為走的是天然精油 + 浸泡油的路線，沒加合成定香劑。需要泡一段時間、靠近聞、跟皮膚接觸後才會慢慢釋放。這也是為什麼慢澡時香氣感受最完整。',
          },
          {
            q: '為什麼有人對某些香味特別喜歡或討厭？',
            a: '多半是過去經驗的累積。喜歡桂花可能是因為小時候家門口有桂花樹；討厭某種草本香可能是某次生病喝中藥的記憶。沒有「對的」「錯的」香 ── 只有「對你的」香。',
          },
          {
            q: '香氣會隨著時間變淡嗎？',
            a: '會。冷製皂熟成 60–90 天香氣最完整、用 30 天後逐漸淡（精油分子隨水分蒸發釋放）── 這是正常的。如果一塊皂你買回來放半年才開始用，前面的香氣已經先「用」了一部分。',
          },
          {
            q: '聞著聞著流淚了正常嗎？',
            a: '正常。普魯斯特效應（《追憶似水年華》裡的描述）── 一塊瑪德蓮蛋糕的味道讓主角想起整個童年。這是嗅覺神經跟記憶的天然連結。流淚不是壞事，是身體在告訴你某段時間真的存在過。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-04-30',
    cover: {
      src: '/images/journal/last-sliver.png',
      alt: '工坊木桌上一小片用到剩極薄的皂塊躺在棉布上，旁邊一個棉麻網袋、一束乾燥薰衣草、一只盛著皂泥的小陶盤',
      fallback: { tone: 'cool', flower: 'lavender' },
    },
    description:
      '一塊皂用到剩一小角，捨不得丟、又不好抓握。本篇講三種延長尾段使用的方法（皂袋、皂接龍、皂泥），跟惜物這件事的小儀式。',
    keywords: ['皂袋', '皂接龍', '皂泥', '惜物', '一塊皂的壽命', '延長使用'],
    related: ['soap-storage', 'gift-soap', 'why-handmade-soap'],
    body: [
      '一塊皂的最後一小角總是會卡住一陣子。',
      '不夠厚搓不出泡、太小拿不穩、丟掉又覺得可惜。多數人就讓它躺在皂盤角落幾天 ── 然後在某個整理浴室的下午，被丟進垃圾桶。',
      '老闆娘鍋邊的習慣不是這樣。一塊皂用到剩多小，都還是有它的位置。',
      { type: 'h2', text: '為什麼會捨不得' },
      '一塊冷製皂從調油、入模、熟成到送到你家，至少 60 天。它在你手裡又洗了 60 天。前後 4 個月的時間 ── 這一小角是這段關係的尾巴。',
      '它不只是一塊皂。它是你冬天某幾個早晨的暖、夏天某些傍晚的涼、某次出差包在毛巾裡帶出去的那塊。它知道你一些事。',
      {
        type: 'figure',
        src: '/images/journal/inline/last-sliver-1.png',
        alt: '從整塊到剩一小角的四階段手繪示意 ── 最後一段被收進一個棉麻網袋裡',
        caption: '一塊皂的四個時期 ── 最後它會回到一個小袋子',
      },
      { type: 'h2', text: '三種方法延長最後一段' },
      '第一、皂袋（mesh drawstring soap-saver bag）── 一個棉麻或網眼的小袋子，把剩下的皂塊收進去拉緊。整個袋子當皂用 ── 起泡更豐、不容易掉、能用到剩米粒大。本舍出貨會附一個給選購了第三塊以上的客人。',
      '第二、皂接龍 ── 新皂剛拆封時表面微濕，把舊皂的小角壓在新皂上，過幾分鐘自然黏合。這個方法是傳統用皂法，把兩塊皂變一塊。',
      '第三、皂泥 ── 把好幾塊用剩的小角集中起來、加少許水軟化、混成皂泥（像黏土那樣）。可以重新塑形成新的小皂塊，或當「家事用皂」洗洗碗、洗洗鞋子、洗洗抹布。',
      { type: 'h2', text: '一個小儀式' },
      '我們鍋邊有一個習慣：一塊皂用完那天會記一筆。哪一塊、什麼時候開始用、什麼時候用完、有沒有什麼想記的。',
      '不是為了統計。是因為一塊皂用了兩個月跟你的浴間時光重疊 ── 它見證了你某段日常。把它的尾巴記下來，是對它的一個小敬意。',
      '工業洗劑沒有這個尾巴 ── 一瓶用完直接丟進回收箱。但一塊冷製皂不一樣，它從來不是「快速消耗品」── 它是日常裡的小陪伴。',
      '你不一定要寫下來。但下次看到剩一小角的皂塊，多看它一眼 ── 那一眼就是告別。',
      {
        type: 'faq',
        items: [
          {
            q: '皂袋哪裡買？',
            a: '本舍出貨給三塊以上的單會附一個。如果是單塊客人想要，可以加 Line 備註，我們再放一個。網路上也很多手作棉麻網袋可以買，挑材質透氣、繩可拉緊的款。',
          },
          {
            q: '皂泥怎麼塑形？',
            a: '小角加少許水（噴霧瓶噴 2 下就夠）放進密封袋揉軟。揉到像麵團就可以塑形 ── 用小餅乾模壓也好、用手揉成圓球也好。塑好後放陰乾 1–2 天，硬了就能用。',
          },
          {
            q: '皂接龍兩種不同款的皂壓在一起會混嗎？',
            a: '會。所以一般選同款或同系列的接（例如海棠接海棠、桂花接桂花）。如果無所謂、純粹想用完，混款也沒問題 ── 只是泡沫跟洗感會折衷。',
          },
          {
            q: '一塊皂能用多久才算「不浪費」？',
            a: '本舍 105 g 的皂，每天洗澡用大約 60–80 天 ── 這個長度才是它配方價值被完整釋放的時間。如果三週用完代表「沒讓它做完事」── 通常是皂盤瀝水不對、放在水池邊。',
          },
        ],
      },
    ],
...

  {
    ...
    date: '2026-05-14',
    cover: {
      src: '/images/journal/slow-bath.png',
      alt: '深色石浴缸邊一塊木板放著皂、毛巾、半開的書、茶杯與燭台 ── 一張屬於慢時光的浴間靜物',
      fallback: { tone: 'warm', flower: 'osmanthus' },
    },
    description:
      '我們很久沒有把洗澡當一件事了。5 分鐘的快洗澡像收尾的工作，30 分鐘的慢洗澡是一段重新跟自己在一起的時間。本篇講為什麼浴間值得慢一點、怎麼準備一場慢洗澡、配方裡的香氣怎麼幫上忙。',
    keywords: ['慢生活', '泡澡', '洗澡儀式', '浴室時光', '芳香沐浴', '冷製皂'],
    related: ['gift-soap', 'scent-and-memory', 'why-handmade-soap'],
    body: [
      '我們很久沒有把洗澡當一件事了。',
      '一天的最後一段時間，多數人是這樣過的 ── 五分鐘解決完，皂沖一沖、頭洗一洗、毛巾擦兩下出來，整個過程跟刷牙差不多。洗澡變成一件「結束的事」，不是「開始的事」。',
      '可是浴間原本不是這樣。在還沒有 24 小時便利店、智慧型手機、隔天早會的年代，洗澡是一段沒有人能打擾的時間 ── 衣服脫了、門關上了、水聲蓋過所有外面的聲音。整個世界暫時退到門外。',
      '老闆娘鍋邊每次寫配方寫到累的時候，會去洗一個慢澡。不是因為髒，是因為需要那段被水包起來的時間。',
      { type: 'h2', text: '5 分鐘 vs 30 分鐘' },
      '5 分鐘的洗澡：水開、皂搓、沖、出來、擦乾。整個動作只有「洗」這個目的。心思還在剛剛的工作信、剛剛的對話、剛剛的待辦事項。',
      '30 分鐘的洗澡：水放好、燈調暗、坐進去先什麼都不做兩分鐘、皂搓開到全身、慢慢沖、再坐一陣子、起來不急著擦乾、披一條毛巾在肩上喝半杯茶 ── 30 分鐘走完，身體跟頭腦都在同一個地方了。',
      '差別不是「乾不乾淨」── 5 分鐘也夠乾淨。差別是「人有沒有從工作模式切回來」。',
      {
        type: 'figure',
        src: '/images/journal/inline/slow-bath-1.png',
        alt: '兩段時間軸對照手繪示意 ── 上方 5 分鐘只有一個短刻度，下方 30 分鐘拉長成弧線，沿線分散畫著書、蠟燭、茶杯與皂',
        caption: '5 分鐘只夠收尾、30 分鐘可以歸位 ── 浴間時光的兩種速度',
      },
      { type: 'h2', text: '一場慢洗澡的小道具' },
      '不需要很多。整套加起來不到一千塊：',
      '一塊皂 ── 不是任何皂都行，是一塊「洗起來不急」的皂。配方裡椰子比例壓低、橄欖跟乳油木果脂多一點，泡沫不衝、洗感慢慢釋放。本舍的桂花潤膚或霧蜜都對。',
      '一支蜜蠟蠟燭 ── 浴間的螢光燈太亮、太「公務」。一支蠟燭就夠了，光從燭芯散出去，水面上會有一點點晃動的反光。',
      '一杯茶或一杯熱開水 ── 泡澡時人體會微脫水，補一點。麥茶、薏仁茶、大麥茶都好，有香氣的飲品比白水更讓人放鬆。',
      '一本實體書 ── 不是手機。手機是工作載體，帶進浴室就還在工作。一本紙本書（不需要讀完），翻幾頁、看一個段落、停下來想一下，這些都是浴間時光的一部分。',
      '可選：一個小木檯架在浴缸上放這些東西 ── bath caddy。沒有也行，浴室角落的小凳子也可以。',
      { type: 'h2', text: '配合身體的節奏' },
      '水溫不要太燙。38–40°C 是身體最舒服的範圍 ── 接近體溫但比體溫稍高，可以泡 20 分鐘不流汗。太燙泡 5 分鐘就受不了，反而短了時光。',
      '洗的順序：先泡（5 分鐘暖身）→ 再洗（皂搓開、慢慢洗，不急著沖）→ 再泡（5–10 分鐘）→ 起來。中間如果想讀書就讀、想閉眼就閉。',
      '起來不急著擦乾。披一條大毛巾在肩上、坐在浴室外的椅子上喝幾口茶，等身體自己散熱 ── 這幾分鐘是慢洗澡的一部分，不是「洗完之後的事」。',
      { type: 'h2', text: '配方裡的香氣怎麼幫忙' },
      '香氣是慢洗澡最隱形的協助。配方裡放了一支對的精油，整個浴間會自己變成一個被氣味包裹的空間 ── 不需要再點別的薰香。',
      '想要安神的：薰衣草、洋甘菊。睡前的慢澡選這兩支，洗完很容易睡著。',
      '想要清醒的：薄荷、迷迭香。週末早上的慢澡（沒事不上班那種），洗完精神特別好。',
      '想要溫暖的：桂花、檀香、安息香。冬夜的慢澡，洗完整個人像被毯子裹著走出來。',
      '本舍的桂月流金（桂花潤膚）是最常被選來慢澡的一塊 ── 桂花的甜香淡而長，泡 20 分鐘味道才完全釋放。',
      '浴間是私人時間中最私人的。把它還給自己。',
      {
        type: 'faq',
        items: [
          {
            q: '我家只有淋浴沒有浴缸，也能慢嗎？',
            a: '可以。慢洗澡的核心不是「泡」是「不急」。淋浴可以慢沖、慢洗、把水溫調到剛好坐著沖背都不冷。重點是把時間拉長到 15–20 分鐘，並且不帶手機進去。',
          },
          {
            q: '不帶手機真的有差嗎？',
            a: '差很多。手機在浴室裡的存在會讓你的注意力一直「半在工作、半在洗澡」 ── 結果兩邊都沒做好。第一次試不帶可能不習慣，三次之後會覺得不要更自由。',
          },
          {
            q: '我有小孩，根本沒有 30 分鐘的私人時光怎麼辦？',
            a: '這正是最需要慢澡的時期。可以跟伴侶輪流值班，一週兩次留一個 30 分鐘的「沒人能找我」時段。把它當成一個小小的休假。',
          },
          {
            q: '慢洗澡會不會洗太久反而傷皮膚？',
            a: '只要水溫在 38–40°C 而非高於 42°C，泡 20 分鐘對皮膚是安全的。如果是冷製皂洗身體，洗的動作仍是 3–5 分鐘 ── 剩下的 15 分鐘是「在水裡不洗」，皮膚不會額外負擔。',
          },
        ],
      },
    ],
...

  {
    ...
    body: [
      ...
      '給長輩 → 桂花、酒粕、海棠。長輩偏愛清淡甜香（不要花露蓋過去）、洗感要溫潤不刺、配方不能太活潑。桂月流金的桂花溫吞、杜康醉月的酒粕滋潤、瓊崖海棠厚實潤澤 ── 三選一都不會踩雷。',
      '給朋友（同齡）→ 看對方的生活狀態。常運動的朋友 → 一皂到底·清爽；夜班族 / 壓力大的朋友 → 蝶豆（清透好洗）；剛搬家的朋友 → 山茶淨髮髮餅（搬家最容易顧不上頭髮）。',
      ...
      '節期禮（年節 / 中秋 / 母親節）→ 三塊組為主。常見搭配：「日常香氣 + 季節滋養 + 家人共用」── 例如桂花 + 海棠 + 大米。',
      ...
      '送一塊皂，是把一段日常給對方。',
      {
        ...
        items: [
          ...
          {
            q: '對方有過敏怎麼辦？',
            a: '事先問一句最保險。若不方便問，挑最低敏的兩款 ── 大米（無香）、海棠（潤澤款，本來就為敏感肌設計）。這兩款踩到敏感的機率最低。',
          },
          {
            q: '送禮要不要附手寫小卡？',
