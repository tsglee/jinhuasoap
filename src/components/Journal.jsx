...
const POSTS = [
...

  {
    ...
    body: [
      ...
      '我們不寫「治療」這件事 ── 那是醫生的領域。但腳跟皂的關係，是日常生活裡可以做點什麼的範圍。',
      ...
      '腳趾縫脫皮、長水泡、發紅 ── 香港腳的典型表現，需要抗黴菌藥膏，不是皂能解決。',
    ...
    body: [
      ...
      '皮膚的節律是身體在跟你說話。聽得到，照顧才會準。',
      {
        ...
        items: [
          ...
          {
            q: '經前長下顎痘可以擠嗎？',
            a: '不建議。荷爾蒙性痘的成因在內，擠了還是會再冒。可以做的：那一週清潔加強、保養減量（少擦乳液避免悶）、發炎處冰敷消紅。',
          },
          {
            q: '吃避孕藥會影響皮膚嗎？',
...

  {
    ...
    description:
      '主婦濕疹（手部濕疹）跟反覆接觸水跟洗劑直接相關。每洗一次手皮膚屏障被剝一層 ── 一天 20 次累積起來就是危機。本篇講手部皮膚的限度、選皂選洗碗精的原則、跟修復節奏。',
    ...
    body: [
      ...
      '冷製手工皂洗手比工業洗手乳溫和很多 ── 因為甘油保留下來，洗完有薄薄一層水分。本舍最常被選來洗手的是稻花暖心（大米皂無香）跟瓊崖海棠（海棠修復皂）。',
      ...
      { type: 'h2', text: '修復 ── 給手時間' },
      '洗完手立刻擦護手乳。3 分鐘內最有效 ── 沖水後手部皮膚是「半濕」狀態，這時擦的乳液鎖水率最高。',
      '晚上睡前的「手部修復夜」── 洗完雙手用蜂蠟膏（或厚一點的潤膚膏）厚塗，戴一雙純棉薄手套睡覺。一週兩次，龜裂的指縫一個月內會修回來。',
      ...
      '一雙手是一輩子的事。它們做的事很多人看不到，但它們知道。',
      {
        ...
        items: [
          {
            q: '我手已經有濕疹了，還能用冷製皂嗎？',
            a: '急性發作期建議先用無香的款（大米）或弱酸皂餅（茉莉沐膚）；穩定期可以回到瓊崖海棠這類修復款。如果濕疹嚴重請看皮膚科 ── 皂是日常輔助，類固醇藥膏才是急性處理。',
          },
          {
            q: '洗碗用海棠這類「貴的皂」會不會浪費？',
          ...
          {
            q: '小孩家事一起做，手會不會也壞？',
            a: '小孩的修復力比成人強 ── 但偶發性的家事（每週一次）不會出問題；天天洗碗反覆接觸洗碗精會。同樣建議：戴手套或用更溫和的洗劑。',
          },
        ],
      },
    ],
...

  {
    ...
    body: [
      ...
      '油本身不是壞 ── 它是皮膚屏障的一部分。會冒痘是因為三件事同時發生：油多 + 角質代謝太快堵住毛孔 + 痤瘡桿菌（C. acnes）在堵住的毛孔裡發炎。三個條件中斷任一個，痘痘就會減少。',
      {
        type: 'figure',
        src: '/images/journal/inline/teen-acne-1.png',
        alt: '毛囊剖面手繪示意 ── 三個狀態並排：清通、油堵、發炎',
        caption: '一個毛囊的三段時間 ── 從通順、被堵、到發炎',
      },
...
,
      ...
      '青春期會過去。一塊溫和的皂、不要洗太勤、別擠痘痘 ── 這三件事做對，渡過去之後皮膚會穩。',
      {
        ...
        items: [
          ...
          {
            q: '冷製皂的鹼性會不會讓痘痘更嚴重？',
            a: '不會 ── 健康年輕皮膚 30 分鐘恢復弱酸沒問題。痘痘的成因是油脂跟發炎，不是 pH。冷製皂選對款（綠豆、蝶豆）反而幫助。',
          },
          {
          ...
          {
            q: '皂用完痘痘還是冒，要不要換？',
            a: '一塊皂連用 4 週才看得出效果。如果 4 週後仍嚴重，先看皮膚科 ── 痘痘有時是需要藥物治療的（特別是發炎到留疤），皂在這個階段是輔助不是主力。',
          },
        ],
      },
    ],
...

  {
    ...
    body: [
      ...
      '原則一：往「保濕厚實」那一端移。如果以前用海棠（中度修復），現在可以選霧蜜（蜂蜜潤厚）；以前用大米（清爽溫和），現在選桂花潤膚或杜康醉月（酒粕滋潤）。',
...

  {
    ...
    body: [
      ...
      '6 個月以後（皮脂膜逐漸成熟）── 可以考慮溫和的冷製皂。本舍的稻花暖心（大米皂）跟瓊崖海棠修復皂是最常被媽媽選的兩款 ── 大米無香、海棠的配方為敏感肌設計。建議仍是先試一塊小範圍、觀察 24 小時，沒紅、沒疹、沒乾再擴大使用。',
      ...
      '這個階段最好用的兩款 ── 瓊崖海棠修復皂（用全身 + 洗臉）跟山茶淨髮髮餅（洗頭髮）。前者修護屏障、後者是 pH 5–6 對 24 小時都接觸寶寶的敏感頭皮溫和。',
