...
function _LegacyLily({ size = 120, tone = 'gold' }) {
  ...
  return (
    ...
    >
      ...
      <path
        d="M100 172 C 102 148, 98 124, 104 100"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w3}
      />
      ...
      <g>
        <path
          d="M100 168 C 82 170, 64 162, 52 146 C 70 152, 86 160, 100 168 Z"
          stroke={`url(#lily-ink-${size}-${tone})`}
          strokeWidth={w2}
        />
        <path
          d="M100 168 C 84 161, 70 154, 56 148"
          stroke={shade}
          strokeWidth={w1}
          opacity="0.7"
        />
      ...
      <g>
        <path
          d="M100 168 C 120 172, 140 168, 156 152 C 138 156, 118 162, 100 168 Z"
          stroke={`url(#lily-ink-${size}-${tone})`}
          strokeWidth={w2}
        />
        <path
          d="M100 168 C 118 162, 134 157, 150 152"
          stroke={shade}
          strokeWidth={w1}
          opacity="0.7"
        />
      ...
      <path d="M94 170 C 100 174, 108 174, 112 170" stroke={ink} strokeWidth={w2} />
      <path d="M95 172 L 111 172" stroke={shade} strokeWidth={w1} opacity="0.7" />

      {/* ── Bloom: 5 petals radiating, three-quarter view ─────────── */}
      {/* Back-left big petal (curls up and back) */}
      <path
        d="M104 100 C 76 96, 52 76, 44 50 C 60 58, 80 78, 104 100 Z"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w2}
      />
      {/* top petal — tall, leaning right */}
      <path
        d="M104 100 C 118 74, 130 44, 128 20 C 140 38, 142 72, 118 98"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w2}
      />
      {/* right upper petal */}
      <path
        d="M104 100 C 132 88, 158 78, 170 60 C 162 82, 144 102, 118 110"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w2}
      />
      {/* front lower-left petal (the big cupped one) */}
      <path
        d="M104 100 C 86 110, 62 118, 46 132 C 68 132, 94 124, 110 110 Z"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w2}
      />
      {/* front lower-right petal */}
      <path
        d="M108 104 C 108 124, 116 140, 134 146 C 134 128, 128 112, 114 104 Z"
        stroke={`url(#lily-ink-${size}-${tone})`}
        strokeWidth={w2}
      />
      ...
      <path d="M92 32 L 96 36 M 90 30 L 98 30" stroke={ink} strokeWidth={w2} />
      ...
      {[
        ...
        <g key={i}>
          ...
          <g transform={`translate(${s.ax} ${s.ay}) rotate(${s.angle})`}>
            <rect x="-5" y="-2.2" width="10" height="4.4" rx="1.8" fill={ink} stroke={ink} />
            {/* pollen speckle */}
...
export function PhotoPlaceholder({ ratio = '4/5', label = 'soap photo', tone = 'warm' }) {
  const palettes = {
    warm: ['#e6d4a3', '#d1b47a'],
    deep: ['#6e5a3a', '#4a3a22'],
    rose: ['#e2b8a8', '#c49689'],
...
export function Divider({ width = '100%', tone = 'gold' }) {
