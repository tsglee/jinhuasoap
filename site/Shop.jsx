...
function Shop() {
  ...
  const updateQty = (num, delta) => {
    setCart(c => c.map(i => i.num === num ? { ...i, qty: Math.max(0, i.qty + delta) } : i).filter(i => i.qty > 0));
  };
  ...
  return (
    <div style={{ position: "relative", zIndex: 1 }}>
      ...
      <section style={{
        ...
        <div className="mono" style={{ color: "var(--red)" }}>Where to find us · 尋皂處</div>
        <h1 className="tc" style={{
          fontSize: 72, fontWeight: 500, letterSpacing: 16,
          margin: "16px 0 10px", color: "var(--sumi)",
        }}>購皂去處</h1>
      ...
      <section style={{
        ...
        <div>
          <div style={{
            ...
            <div>
              <div className="mono" style={{ color: "var(--gold-3)" }}>Stockists · 店家</div>
              <div className="tc" style={{
                fontSize: 32, letterSpacing: 6, marginTop: 6,
              }}>實體店面</div>
            </div>
            ...
            <svg viewBox="0 0 400 225" style={{ width: "100%", height: "100%" }}>
              <defs>
                <pattern id="map-grid" width="20" height="20" patternUnits="userSpaceOnUse">
                  <path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(26,21,18,0.06)" strokeWidth="0.5" />
                </pattern>
              </defs>
              <rect width="400" height="225" fill="url(#map-grid)" />

              {/* Tamsui / Keelung river — abstract curves through Taipei basin */}
              <path d="M40 70 C 100 60, 160 90, 220 80 C 280 72, 340 100, 380 90"
                fill="none" stroke="rgba(77,107,75,0.35)" strokeWidth="3"/>
              <path d="M200 40 C 210 80, 180 130, 200 190"
                fill="none" stroke="rgba(77,107,75,0.35)" strokeWidth="3"/>

              {/* Basin outline — soft rounded blob */}
              <path d="M50 60 C 40 120, 70 180, 160 195 C 260 205, 360 170, 370 110 C 375 60, 300 30, 200 35 C 120 40, 60 40, 50 60 Z"
                fill="rgba(198,154,58,0.12)" stroke="var(--gold-3)" strokeWidth="0.8"/>
              ...
              {[
                ...
                <text key={i} x={d.x} y={d.y}
                  ...
                  fill="rgba(26,21,18,0.35)" textAnchor="middle">{d.label}</text>
              ))}

              {/* Pins — one per stockist */}
              {[
                ...
                <g key={i}>
                  ...
                  <circle cx={p.x} cy={p.y} r={p.kind === "flag" ? 12 : 9}
                    fill="none" stroke="var(--red)" opacity="0.3"/>
                  {p.kind === "flag" && (
                    <circle cx={p.x} cy={p.y} r="3" fill="var(--gold-2)"/>
                  )}
                  <text x={p.x + 10} y={p.y + 3}
                    fontFamily='"Noto Serif TC", serif' fontSize="10"
                    fill="var(--sumi)">{p.label}</text>
                </g>
              ))}
              ...
              <g transform="translate(370, 30)">
                ...
                <text x="0" y="-16" textAnchor="middle" fontFamily='"DM Mono", monospace' fontSize="7" fill="var(--gold-3)">N</text>
                <path d="M0 8 L-3 0 L0 -8 L3 0 Z" fill="var(--red)"/>
              </g>

              {/* Title */}
              <text x="24" y="30" fontFamily='"Cormorant Garamond", serif' fontStyle="italic"
                fontSize="14" fill="var(--gold-3)">Taipei · 台北</text>
            </svg>
            <div style={{
              position: "absolute", bottom: 14, left: 24,
              fontFamily: '"DM Mono", monospace', fontSize: 9,
              letterSpacing: 2, color: "var(--ink-60)", textTransform: "uppercase",
            }}>5 stockists · all within taipei city</div>
          </div>
          ...
          <div style={{
            ...
            {filtered.map((s, i) => (
              <div key={i} style={{
                ...
                <div>
                  <div className="tc" style={{ fontSize: 20, letterSpacing: 3 }}>{s.zh}</div>
                  <div className="italic" style={{ fontSize: 13, color: "var(--gold-3)" }}>Taipei</div>
                </div>
                <div>
                  <div style={{ fontSize: 15, color: "var(--sumi)" }}>{s.addr}</div>
                  <div className="mono" style={{ color: "var(--ink-60)", marginTop: 4 }}>{s.hours}</div>
                </div>
                <div style={{
                  padding: "4px 10px",
                  fontFamily: '"DM Mono", monospace', fontSize: 9,
        ...
        <aside style={{
          ...
          <div style={{
            ...
            <div>
              <div className="mono" style={{ color: "var(--red)" }}>Shop online · 線上</div>
              <div className="tc" style={{ fontSize: 26, letterSpacing: 4, marginTop: 4 }}>您的籃子</div>
            </div>
          ...
          {cart.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 0", color: "var(--ink-60)", fontStyle: "italic" }}>
          ...
          {cart.map(item => (
            <div key={item.num} style={{
              ...
              <div>
                <div className="tc" style={{ fontSize: 18, letterSpacing: 3 }}>{item.zh}</div>
                <div className="italic" style={{ fontSize: 13, color: "var(--gold-3)" }}>{item.en} · No. {item.num}</div>
                <div style={{ display: "flex", gap: 6, marginTop: 6, alignItems: "center" }}>
                  <button onClick={() => updateQty(item.num, -1)} style={{
                    width: 22, height: 22, border: "1px solid var(--ink-15)",
                    fontFamily: '"DM Mono", monospace', fontSize: 12,
                  }}>−</button>
                  <span style={{ fontFamily: '"DM Mono", monospace', fontSize: 12, minWidth: 22, textAlign: "center" }}>{item.qty}</span>
                  <button onClick={() => updateQty(item.num, 1)} style={{
                    width: 22, height: 22, border: "1px solid var(--ink-15)",
                    fontFamily: '"DM Mono", monospace', fontSize: 12,
                  }}>+</button>
          ...
          {cart.length > 0 && (
            <>
              <div style={{ marginTop: 20, display: "grid", gap: 8, fontFamily: '"DM Mono", monospace', fontSize: 11, letterSpacing: 1 }}>
                ...
                <div style={{
                  ...
                  paddingTop: 10, marginTop: 6, borderTop: "1px solid var(--sumi)",
                  fontFamily: '"Cormorant Garamond", serif', fontStyle: "italic",
                  fontSize: 20,
                }}>
                  <span>Total</span>
              ...
              <div className="italic" style={{
                marginTop: 14, fontSize: 13, color: "var(--ink-60)", textAlign: "center",
              }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div>
            <div className="mono" style={{ color: "var(--gold-2)" }}>Wholesale · 批發合作</div>
            <h2 className="tc" style={{
              fontSize: 44, fontWeight: 400, letterSpacing: 8,
              margin: "12px 0", color: "var(--paper)",
            }}>開店合作</h2>
            <div className="italic" style={{ fontSize: 18, color: "rgba(236,227,207,0.85)", maxWidth: 440, lineHeight: 1.6 }}>
              We partner with a small number of independent shops. If your counter would feel at home with our soap, write to us — minimum order twelve bars.
            </div>
            <button style={{
              marginTop: 28,
              ...
              letterSpacing: 3, textTransform: "uppercase",
            }}>Apply to stock · wholesale@goldenflower.tw</button>
          </div>

          <div>
            <div className="mono" style={{ color: "var(--gold-2)" }}>Journal · 月訊</div>
            <h2 className="tc" style={{
              fontSize: 44, fontWeight: 400, letterSpacing: 8,
              margin: "12px 0", color: "var(--paper)",
            }}>月信</h2>
            <div className="italic" style={{ fontSize: 18, color: "rgba(236,227,207,0.85)", maxWidth: 440, lineHeight: 1.6 }}>
              One letter a month — new batches, the ingredient we're thinking about, and a small essay from the cure room. No sales, no plastic talk.
            </div>
            <form onSubmit={(e) => { e.preventDefault(); setAddingEmail(""); }}
